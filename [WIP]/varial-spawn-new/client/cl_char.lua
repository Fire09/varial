            ...
            Login.RequestModel(cModelHash, function(loaded, model, modelHash)
                if loaded then
                    ...
                    if not isCustom then
                        if modelHash == GetHashKey("mp_f_freemode_01") or modelHash == GetHashKey("mp_f_freemode_01") then
                            if character ~= nil then
...
RegisterNetEvent("varial-spawn-new:finishedClothing")
AddEventHandler("varial-spawn-new:finishedClothing", function(endType)
