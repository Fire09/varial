...
AddEventHandler('spawn:clientSpawnData', function(spawnData)
		...
		if spawnData.overwrites ~= nil then
			...
			elseif spawnData.overwrites == "new" then
				...
				-- TriggerEvent("varial-spawn-new:firstspawnyo")
			end
			return
		end
		...
		--[[if Spawn.housingCoords == nil then
			Spawn.housingCoords = exports["varial-housing"]:retriveHousingTable();
		end]]
...
RegisterNetEvent("varial-spawn-new:firstspawnyo")
AddEventHandler("varial-spawn-new:firstspawnyo", function()
    if GetEntityModel(PlayerPedId()) == GetHashKey("mp_f_freemode_01") then
        TriggerEvent("varial-spawn-new:finishedClothingstuffbish")
    else
        TriggerEvent("varial-spawn-new:finishedClothingstuff")
    end
end)

RegisterNetEvent('varial-spawn-new:finishedClothingstuff')
AddEventHandler('varial-spawn-new:finishedClothingstuff', function()
...
RegisterNetEvent('varial-spawn-new:finishedClothingstuffbish')
AddEventHandler('varial-spawn-new:finishedClothingstuffbish', function()
...
function Spawn.getBusinessSpawn(pBusinessCode, pBusinessName)
	...
	local hasPermission = RPC.execute("varial-business:hasPermission", pBusinessCode, "key_access")
...
function Spawn.selectedSpawn(spawnInfo)
	...
	if apartment then
		...
		if pos then
			...
			DoScreenFadeIn(2500)
			TriggerEvent("varial-spawn-new:characterSpawned")
			TriggerEvent("varial-spawn-new:characterSpawned")
		else 
