...
TriggerServerEvent('server:givepass')
...
RegisterNUICallback('editEmployee', function(data)
  ...
  local complete = RPC.execute('varial-newphone:business_edit', business,cid,rankid)
...
RegisterNUICallback('fireEmployee', function(data)
    ...
    local complete = RPC.execute('varial-newphone:business_fire', business,cid)
...
RegisterNUICallback('payCheckEmployee', function(data)
    ...
    local complete, paid = RPC.execute('varial-newphone:business_paycheck', business,cid,amount)
    if complete then
...
RegisterNUICallback('btnTaskGroups', function()
    ...
    -- local groups,bank,gIp = RPC.execute("varial-newphone:Groups")
    SendNUIMessage({
      openSection = "groups",
      groups = groupObject
    })
  ...
  RegisterNUICallback('varial-newphone:fireEmp', function(data,cb)
    TriggerServerEvent('varial-newphone:fireEmp', data.id,data.group,data.name)
  end)

  RegisterNUICallback('addCashBank', function(data)
    local bid = data.bid
    local amount = data.amount
    local bank = RPC.execute("varial-newphone:bus_addBank",bid,amount)
    if bank then
