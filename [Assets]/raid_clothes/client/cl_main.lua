...
local MenuData = {
    clothing_shop = {
        text = "Get your Drip up",
        displayName = "Clothing Store",
        basePrice = 200
    },
    barber_shop = {
        text = "Become Kim Kardashian",
...
function Save(save, close)
    ...
    if save then
        ...
        if not startingMenu or passedClothing then
            ...
            TriggerEvent("varial-spawn:finishedClothing","Finished")
        ...
        TriggerEvent("varial-spawn:finishedClothing","Old")
        LoadPed(oldPed)
    end
    ...
    TriggerEvent('varial-weapons:getAmmo')
...
RegisterUICallback("varial-ui:raid_clothes:addOutfitPrompt", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    Wait(1) --wait to fix ui bug?
    exports['varial-ui']:openApplication('textbox', {
        callbackUrl = 'varial-ui:raid_clothes:addOutfit',
        header = "Add Outfit",
        key = data.key,
        items = {
            {
...
RegisterUICallback("varial-ui:raid_clothes:addOutfit", function(data, cb)
...
RegisterUICallback("varial-ui:raid_clothes:changeOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    TriggerEvent('hotel:outfit', { true, data.key }, 3)
    exports["varial-ui"]:hideContextMenu()
end)

RegisterUICallback("varial-ui:raid_clothes:deleteOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
...
AddEventHandler('raid_clothes:ListOutfits', function(skincheck)
    ...
    for i = 1, #skincheck do
        ...
        menuData[#menuData + 1] = {
            ...
            key = slot,
            children = {
                { title = "Change Outfit", action = "varial-ui:raid_clothes:changeOutfit", key = slot},
                { title = "Delete Outfit", action = "varial-ui:raid_clothes:deleteOutfit", key = slot},
            }
        }
    end
    if #menuData >= 0 then
        if #menuData < 10 then
            ...
            menuData[#menuData + 1] = {
                ...
                description = '',
                key = emptySlot,
                action = "varial-ui:raid_clothes:addOutfitPrompt"
            }
        end
...
function SetCustomNuiFocus(hasKeyboard, hasMouse)
  ...
  -- TriggerEvent("varial-voice:focus:set", HasNuiFocus, hasKeyboard, hasMouse)
end
...
AddEventHandler("varial-polyzone:enter", function(zone, data)
    ...
    if currentZone then
        exports['varial-interaction']:showInteraction(("[M] %s"):format(currentZone.text))
...
AddEventHandler("varial-inventory:itemUsed", function(item)
...
RegisterNetEvent("varial-context:closeglobal")
AddEventHandler("varial-context:closeglobal", function()
    ...
    local data = {
        {
            ...
            description = "",
            key = true,
            action = "varial-ui:raid_clothes:CustomaddOutfitPrompt"
        },
    }
...
RegisterUICallback("varial-ui:raid_clothes:CustomaddOutfitPrompt", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    Wait(1) --wait to fix ui bug?
    exports['varial-ui']:openApplication('textbox', {
        callbackUrl = 'varial-ui:raid_clothes:addCustomOutfit',
        key = data.key,
        items = {
            {
                icon = "pencil-alt",
...
RegisterUICallback("varial-ui:raid_clothes:addCustomOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    exports['varial-ui']:closeApplication('textbox')
...
-- TO DO : Change the inventory picture on shared list , Add a custom chain texture  
  ...
  RegisterNetEvent("ghost:togglechain") -- hoodlums
...
RegisterNetEvent("ghost:togglechain2") -- hoodlums
AddEventHandler("ghost:togglechain2", function()
  local playerPed = PlayerPedId()
  local allowedmodels = AllowedPeds[GetEntityModel(PlayerPedId())]
  if allowedmodels then
  if not isChainOn then
...
RegisterNetEvent('varial:clothing:admin', function()
    OpenMenu("clothing_shop")
end)
...
RegisterCommand("h1", function(source, args, rawCommand)
    if exports['varial-inventory']:hasEnoughOfItem('hat', 1) then
...
RegisterCommand("m1", function(source, args, rawCommand)
    if exports['varial-inventory']:hasEnoughOfItem('mask', 1) then
