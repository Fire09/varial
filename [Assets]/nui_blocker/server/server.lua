...
local kick_msg = "Cannot use Dev Tools here bozo"
...
function sendToDiscord (source,message,color,identifier)
    ...
    PerformHttpRequest(logs, function(err, text, headers) end, 'POST', json.encode({username = "Evan - NUI Blocker", embeds = sendD}), { ['Content-Type'] = 'application/json' })
end
