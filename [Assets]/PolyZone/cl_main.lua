...
RegisterNetEvent('varial-base:PolyZoneUpdate')
AddEventHandler('varial-base:PolyZoneUpdate', function()
    local combo = ComboZone:Create({}, {name = "varial-polyzone", debugPoly = false})
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('varial-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                --print('varial-polyzone:ssadasdsadsda2')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('varial-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
               -- print('varial-polyzone:stop2')
            end
        end
    end)
...
AddEventHandler("onResourceStart", function(resourceName)
    ...
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('varial-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
              --  print('varial-polyzone:wedasdsadasdsadsadsa')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('varial-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
               -- print('varial-polyzone:stop')
            end
        end
