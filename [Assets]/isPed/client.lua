...
AddEventHandler("varial-jobmanager:playerBecameJob", function(jobpassed, name, notify)
	job = jobpassed
    if not job then
        job = "None"
    end
...
AddEventHandler('varial-base:addedMoney', function(blah,amount)
  cash = math.ceil(amount)
end)

RegisterNetEvent('varial-base:removedMoney')
AddEventHandler('varial-base:removedMoney', function(blah,amount)
  cash = math.ceil(amount)
end)
...
RegisterNetEvent('Void-phone:fix-dead')
AddEventHandler('Void-phone:fix-dead', function()
  dead = false
end)