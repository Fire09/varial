...
BNEBoosting['functions'] = {
    GetCurrentBNE = function()
        if Config['General']["Core"] == "QBCORE" then
            CoreName.Functions.TriggerCallback('ethicalpixel-boosting:getCurrentBNE', function(amount)
                value =  amount.BNE
        ...
        elseif Config['General']["Core"] == "ESX" then
            ESX.TriggerServerCallback('ethicalpixel-boosting:getCurrentBNE', function(amount)
                value =  amount.BNE
                background =  amount.background
            end)
            Wait(200)
    ...
    RemoveBNE = function(amount)
        if Config['General']["Core"] == "QBCORE" then
            CoreName.Functions.TriggerCallback('ethicalpixel-boosting:removeBNE', function(amount) 
                end , amount)
                Wait(200)
            return true
        elseif Config['General']["Core"] == "ESX" then
            ESX.TriggerServerCallback('ethicalpixel-boosting:removeBNE', function(amount)
                end , amount)
                Wait(200)
            return true
        elseif Config['General']["Core"] == "NPBASE" then
    ...
    AddBne = function(amount)
        if Config['General']["Core"] == "QBCORE" then
            CoreName.Functions.TriggerCallback('ethicalpixel-boosting:addBne', function(amount)
                end , amount)
                Wait(200)
            return true
        elseif Config['General']["Core"] == "ESX" then
            ESX.TriggerServerCallback('ethicalpixel-boosting:addBne', function(amount)
                Wait(200)
                return true
            end , amount)
        elseif Config['General']["Core"] == "NPBASE" then
