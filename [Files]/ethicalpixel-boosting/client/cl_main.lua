...
RegisterCommand("wa", function()
  RandomVehicleSelection()
end)

function RandomVehicleSelection()
  for k,v in pairs(Config.Vehicles) do
    local randomint = math.random(1,k)
      print(Config.Vehicles[randomint].vehicle)
      print(Config['Utils']["ClassPrices"][Config.Vehicles[randomint].class])
    --return 
  end
...
AddEventHandler('ethicalpixel-boosting:CreateContract' , function(shit)
  ...
  local response = CreateVeh(dick , vector3(3320.7292, -86.1342, 11.8453))
  ...
  if Config['General']["Core"] == "QBCORE" then
    if Config['General']["MinPolice"] == 0 then
      if started or vinstarted then
        if Config['General']["Core"] == "QBCORE" then
          CoreName.Functions.Notify(Config['Utils']["Notifications"]["AlreadyStarted"], "error", 3500)   
        elseif Config['General']["Core"] == "ESX" then
          ShowNotification(Config['Utils']["Notifications"]["AlreadyStarted"],'error')
        elseif Config['General']["Core"] == "NPBASE" then
          TriggerEvent("DoLongHudText", "AlreadyStarted", 2)
        end
      else
      ...
      if result >= Config['General']["MinPolice"] then
        if started or vinstarted then
          ...
          elseif Config['General']["Core"] == "ESX" then
            ShowNotification(Config['Utils']["Notifications"]["AlreadyStarted"],'error')
          elseif Config['General']["Core"] == "NPBASE" then
            TriggerEvent("DoLongHudText", "All ready Started", 2)
          end
        else
          TriggerEvent("ethicalpixel-boosting:StartContract" , data.id)
          Contracts[data.id].started = true
      ...
      if(veh ~= 0) then
        ...
        if(GetVehicleNumberPlateText(veh) == Contracts[startedcontractid].plate) then
          ...
          if (Config['Utils']["Contracts"]["DisableTrackingOnDCB"]) and (Class == "D" or Class == "C" or Class == "B") then
            ...
            elseif Config['General']["Core"] == "NPBASE" then
                TriggerEvent("DoLongHudText", "No Tracker On This Vehicle", 2)
            end
          elseif(vinstarted == false) then
            if(DisablerTimes < Config['Utils']["Disabler"]["NormalDropOff"]) then
              DisablerUsed = true
              local minigame = exports["varial-minigame"]:Open()    
              if(minigame == true) then
                ...
                elseif Config['General']["Core"] == "NPBASE" then
                  TriggerEvent("DoLongHudText", "Success Hack", 1)
                end
              end
            else
              if(DisablerTimes == Config['Utils']["Disabler"]["NormalDropOff"]) then
                ...
                elseif Config['General']["Core"] == "NPBASE" then
                    TriggerEvent("DoLongHudText", "Tracker Removed VIN Mission", 1)
                end
              end
            end
        ...
        if Config['General']["Core"] == "QBCORE" then
          ...
          elseif Config['General']["Core"] == "NPBASE" then
              TriggerEvent("DoLongHudText", "No Mission Detected", 2)
          end
        end
      end
    end
