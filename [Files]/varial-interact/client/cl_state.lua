...
function GetPedContext(pEntity, pContext)
    ...
    if pContext.flags['isJobEmployer'] then
        pContext.job = exports['varial-jobs']:GetNPCJobData(npcId)
    end
end
...
function GetEntityContext(pEntity, pEntityType, pEntityModel)
    ...
    if context.type == 1 then
        context.flags = exports['varial-flags']:GetPedFlags(pEntity)
    ...
    elseif context.type == 2 then
        --context.meta = exports['varial-vehicles']:GetVehicleMetadata(pEntity)
        context.flags = exports['varial-flags']:GetVehicleFlags(pEntity)
    elseif context.type == 3 then
        context.flags = exports['varial-flags']:GetObjectFlags(pEntity)
    end
...
function IsMenuWanted(pMenu, pEntity)
    return pMenu == 'general' or pMenu == 'peds' and pEntity == 1 or pMenu == 'vehicles' and pEntity == 2 or pMenu == 'objects' and pEntity == 3 or pMenu =='news' and isNews or pMenu == 'k9' and isPolice or isSheriff or isState or pMenu == 'judge' and isJudge
end
...
function hasKeys(pEntity)
    ...
    local hasKeys = exports['varial-vehicles']:HasVehicleKey(pEntity)
...
function hasJobPermission(pJob, pPermission)
  ...
  local hasJobPermission = function()
    if pPermission == "employee" then
      ...
      return RPC.execute('varial-business:hasPermission', pJob, pPermission)
    end
  end
...
RegisterNetEvent("varial-interact:bustVehicleOwnerShipCache", function(pEntity)
...
function getVehicleOwner(pEntity)
  ...
  local vin = exports['varial-vehicledegrade']:GetVehicleIdentifier(pEntity)
  local vehicleOwner = RPC.execute("varial-ottosauto:getVehicleOwner", vin)
...
AddEventHandler("varial-jobmanager:playerBecameJob", function(job, name, notify)
    ...
    if isState and job ~= "state" then isState = false end
    if isSheriff and job ~= "sheriff" then isSheriff = false end
    ...
    if job == "state" then isState = true end
    if job == "sheriff" then isSheriff = true end
...
RegisterNetEvent('varial-jobs:jobChanged')
AddEventHandler('varial-jobs:jobChanged', function(pJobId)
    CurrentJob = pJobId
...
AddEventHandler("varial-polyzone:enter", function(zone, data)
    if zone == "varial-jobs:impound:dropOff" then IsImpoundDropOff = true end
...
AddEventHandler("varial-fishing:currentlyRentedBoat", function(handle)
    currentlyRentedBoat = handle
end)

AddEventHandler("varial-interact:openFridge", function()
  TriggerEvent("server-inventory-open", "31", "Craft");
end)

AddEventHandler("varial-interact:grabDrink", function(args, entity, context)
