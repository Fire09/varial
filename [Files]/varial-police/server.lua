...
AddEventHandler("police:targetCheckInventory", function(target, status)
	local src = source
	local user2 = exports["varial-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('police:jailGranted', function(args)
	...
	local target = exports["varial-base"]:getModule("Player"):GetUser(player)
	local character = target:getCurrentCharacter()
	local playerName = character.first_name .. ' ' .. character.last_name

	local pdunit = exports["varial-base"]:getModule("Player"):GetUser(src)
	...
	exports["varial-base"]:getModule("JobManager"):SetJob(target, "unemployed", true)
end)
...
AddEventHandler('police:dnaAsk', function(t)
	local src = source
	local user2 = exports["varial-base"]:getModule("Player"):GetUser(t)
...
AddEventHandler("911", function(args)
	...
	for k,v in pairs(users) do
		...
		if job == "ems" or job == "police" or job == "sheriff" or job == "state" then
...
AddEventHandler("911r", function(args)
	...
	local canRun = (job == "police" or job == "ems" or job == "sheriff" or job == "state") and true or false
...
RegisterServerEvent('varial-base:Licenses')
AddEventHandler('varial-base:Licenses', function()
    local src = source
	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
...
RegisterServerEvent('varial-base:updatedphoneLicenses')
AddEventHandler('varial-base:updatedphoneLicenses', function()
	local src = source
	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
	local licenceTable = {}
...
RegisterServerEvent('varial-base:addLicenses')
AddEventHandler('varial-base:addLicenses', function()
	local src = source
	local user2 = exports["varial-base"]:getModule("Player"):GetUser(src)
	local char2 = user2:getCurrentCharacter()
...
RegisterServerEvent('varial-base:giveLicenses')
AddEventHandler('varial-base:giveLicenses', function(t)
	local src = source
	local user2 = exports["varial-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
...
RegisterServerEvent('varial-base:removeLicenses')
AddEventHandler('varial-base:removeLicenses', function(t)
	local src = source
	local user2 = exports["varial-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
...
RegisterServerEvent('varial-base:giveLicenseshunt')
AddEventHandler('varial-base:giveLicenseshunt', function(t)
	local src = source
	local user2 = exports["varial-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
...
RegisterServerEvent('varial-base:removeLicenseshunt')
AddEventHandler('varial-base:removeLicenseshunt', function(t)
	local src = source
	local user2 = exports["varial-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
...
RegisterServerEvent('varial-base:giveLicensesfish')
AddEventHandler('varial-base:giveLicensesfish', function(t)
	local src = source
	local user2 = exports["varial-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
...
RegisterServerEvent('varial-base:removeLicensesfish')
AddEventHandler('varial-base:removeLicensesfish', function(t)
	local src = source
	local user2 = exports["varial-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
...
AddEventHandler('police:getLicenses', function(t)
	local src = source
	local user = exports["varial-base"]:getModule("Player"):GetUser(t)
	local char = user:getCurrentCharacter()
...
RegisterCommand('bill', function(source, args)
    local src = source
	local user = exports["varial-base"]:getModule("Player"):GetUser(tonumber(args[1]))
	local character = user:getCurrentCharacter()
	local user2 = exports["varial-base"]:getModule("Player"):GetUser(src)
	local character2 = user2:getCurrentCharacter()
	exports.oxmysql:execute('SELECT job FROM jobs_whitelist WHERE cid = ?', {character2.id}, function(result)
		if result[1].job == 'police' or result[1].job == 'sheriff'  or result[1].job == 'state' or result[1].job == 'judge' then
...
RegisterCommand('callsign', function(source, args)
	...
	exports.oxmysql:execute('SELECT job FROM jobs_whitelist WHERE cid = ?', {character.id}, function(result)
		if result[1].job == 'police' or result[1].job == 'sheriff'  or result[1].job == 'state' or result[1].job == 'ems' then
...
RegisterCommand("hireHeat", function(source)
	local src = source
	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
	exports.oxmysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "heat" and v.rank >= 5 then
				TriggerClientEvent('varial-jobsystem:heatJobMenuHire', src)
			end
		end
	end)
end)

RegisterCommand("fireHeat", function(source)
	local src = source
	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
	exports.oxmysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "heat" and v.rank >= 5 then
				TriggerClientEvent('varial-jobsystem:heatJobMenuFire', src)
			end
		end
	end)
end)
...
RegisterCommand("hirecasino", function(source)
	local src = source
	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
	exports.oxmysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "casino" and v.rank >= 4 then
				TriggerClientEvent('varial-jobsystem:casinoJobMenuHire', src)
			end
		end
	end)
end)

RegisterCommand("firecasino", function(source)
	local src = source
	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
	exports.oxmysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "casino" and v.rank >= 4 then
				TriggerClientEvent('varial-jobsystem:casinoJobMenuFire', src)
			end
		end
	end)
end)
...
RegisterCommand("hireyakuza", function(source)
	local src = source
	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
	exports.oxmysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "yakuza" and v.rank >= 5 then
				TriggerClientEvent('varial-jobsystem:yakuzaJobMenuHire', src)
			end
		end
	end)
end)

RegisterCommand("fireyakuza", function(source)
	local src = source
	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
	exports.oxmysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "yakuza" and v.rank >= 5 then
				TriggerClientEvent('varial-jobsystem:yakuzaJobMenuFire', src)
			end
		end
	end)
end)

-- Bondi Boys Hire / Fire-------------------------------------------------------------

RegisterCommand("hirebondi", function(source)
	local src = source
	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
	exports.oxmysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "bondi_boys" and v.rank >= 4 then
				TriggerClientEvent('varial-jobsystem:bondiJobMenuHire', src)
			end
		end
	end)
end)

RegisterCommand("firebondi", function(source)
	local src = source
	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
	exports.oxmysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "bondi_boys" and v.rank >= 4 then
				TriggerClientEvent('varial-jobsystem:bondiJobMenuFire', src)
			end
		end
	end)
end)

RegisterServerEvent('varial-imp:ImpoundCar')
AddEventHandler('varial-imp:ImpoundCar', function(licensePlate)
	exports.oxmysql:execute("UPDATE characters_cars SET vehicle_state = @state, current_garage = @garage, coords = @coords WHERE license_plate = @plate", {['garage'] = 'Impound Lot', ['state'] = 'Normal Impound', ['plate'] = plate})
	Citizen.Wait(500)
	TriggerEvent("garages:reset:player:function", licensePlate)
end)
