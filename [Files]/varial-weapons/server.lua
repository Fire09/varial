RegisterNetEvent('varial-weapons:getAmmo')
AddEventHandler('varial-weapons:getAmmo', function()
    local ammoTable = {}
    local src = source
    ...
    exports.oxmysql:execute("SELECT type, ammo FROM characters_weapons WHERE id = @id", {['id'] = char.id}, function(result)
        ...
        TriggerClientEvent('varial-items:SetAmmo', src, ammoTable)
    end)
end)

RegisterNetEvent('varial-weapons:updateAmmo')
AddEventHandler('varial-weapons:updateAmmo', function(newammo,ammoType,ammoTable)
    local src = source
	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
