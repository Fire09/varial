RegisterNetEvent("varial-police:showBadge")
AddEventHandler("varial-police:showBadge", function(pSource, pInventoryData)
...
--[[ 			name = pInventoryData.Name .. " " .. pInventoryData.Surname, ]]
	...
	if GetPlayerServerId(PlayerId()) == pSource then
        ...
        local result = Citizen.Await(animation:start(function(self)
            ...
            if not exports["varial-base"]:getVar("dead") and vehicle ~= 0 then
              	TaskLeaveVehicle(self.ped, vehicle, 1)
            elseif exports["varial-base"]:getVar("dead") and vehicle ~= 0 then
              	ClearPedTasksImmediately(self.ped)
              	self:abort()
            elseif exports["varial-base"]:getVar("dead") or IsPedRagdoll(self.ped) then
              	self:abort()
            end
        end))
