...
function correct(){
    ...
    $.post('https://varial-ui/varial-ui:taskBarSkillResult', JSON.stringify({
        success: true,
    }));
}
...
const CloseMenu = () => {
    ...
    saved = "";
    textboxactive = false
    $.post(`https://varial-ui/guccimanecancel`)
...
const PostData = (data) => {
    return $.post(`https://varial-ui/`+textmenucallbackurl, JSON.stringify(data))
}

function closesettings(){
    $(".settings").hide();
    $.post('https://varial-ui/close');
}
...
$(document).on('mousedown', (event) => {
    ...
    if (element.id.split("-")[0] === 'target') {
        ...
        $.post('https://varial-ui/varial-ui:targetSelectOption', JSON.stringify({
            entity : entitydata,
            option :{
                event: eventData,
...
$(document).keydown(function(event) {
    ...
    if (key == 27) {
        if (contextmenuactive) {
            ...
            $(".buttoncontext").html("");
            $(".containercontext").hide();
            $.post('https://varial-ui/closecontext', JSON.stringify({}));
        }
        if (textboxactive) {
            CancelMenu();
        }
        if (eyeintactive){
            eyeintactive = false
            $.post('https://varial-ui/closeeyeint', JSON.stringify({}));
        }
    }
});
        ...
        if (event.target.id == "backmenu") {
            ...
            if (event.target.id.split("-")[0] === 'contextmenu') {
                ...
                if (children) {
                    ...
                    $(".buttoncontext").html("");
                    $(".containercontext").hide();
                    $.post('https://varial-ui/'+action, JSON.stringify({
                        key:key
                    }));
                ...
                $(".buttoncontext").html("");
                $(".containercontext").hide();
                $.post('https://varial-ui/'+subaction, JSON.stringify({
                    key:subkey
                }));
            }
        }
