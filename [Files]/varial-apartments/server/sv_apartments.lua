...
AddEventHandler("apartment:serverUnlockAprts",function(roomType)
    ...
    exports.oxmysql:execute("select * from character_motel WHERE cid=@cid",{
        ...
        if #result > 0 then
            if result[1].building_type == roomType then
                TriggerClientEvent("varial-apps:unlocks1",src)
            else
                TriggerClientEvent("varial-apps:unlocks2",src)
            end
        end
    end)
end)
...
RegisterServerEvent('varial-apartments:SetNewRouting')
AddEventHandler('varial-apartments:SetNewRouting', function(roomNumber)
    local src = source
    SetPlayerRoutingBucket(src, roomNumber)
end)

RegisterServerEvent('varial-apartments:GetRouting')
AddEventHandler('varial-apartments:GetRouting', function()
    local src = source
    GetPlayerRoutingBucket(src)
end)

RegisterServerEvent('varial-apartments:SetOldRouting')
AddEventHandler('varial-apartments:SetOldRouting', function()
    local src = source
