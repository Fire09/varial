...
function Apart.enterMotel(roomNumber,roomType,isSpawn)
    ...
    if not Apart.currentRoomLocks[roomType][roomNumber] and Apart.currentRoomLockDown[roomType][roomNumber] then
        if myjob ~= "police" and myjob ~= "state" and myjob ~= "sheriff" and myjob ~= "judge" then
    ...
    TriggerServerEvent('varial-apartments:SetNewRouting', roomNumber)
...
function Apart.processBuildType(numMultiplier,roomType,isSpawn)
    DoScreenFadeOut(1)
    TriggerServerEvent('varial-apartments:GetRouting')
    TriggerEvent("inhotel",true)
    local name = ""
    name = Apart.FindCurrentRoom(roomType)

    local isBuiltCoords = exports["varial-build"]:getModule("func").buildRoom(name,numMultiplier,false)
    ...
    if isBuiltCoords then
        ...
        if isSpawn then
            Apart.WakeFromBed()
            TriggerEvent("varial-spawn:characterSpawned")
        else
...
function Apart.leaveApartment()
    ...
    if Apart.exitPoint[Apart.currentRoomType] == nil then
        exports["varial-build"]:getModule("func").exitCurrentRoom(Apart.Locations[Apart.currentRoomType][Apart.currentRoomNumber])
    else
        if type(Apart.exitPoint[Apart.currentRoomType]) == typeVector then
            exports["varial-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType])
        else
            local rnd = math.random(1,#Apart.exitPoint[Apart.currentRoomType])
            exports["varial-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType][rnd])
        end
    end
    ...
    TriggerEvent("dooranim")
    TriggerEvent('InteractSound_CL:PlayOnOne','DoorClose', 0.7)
    TriggerServerEvent('varial-apartments:SetOldRouting')
    Citizen.Wait(100)
    TriggerServerEvent('varial-apartments:GetRouting')
end
...
--     exports["varial-build"]:getModule("func").CleanUpArea()
--     local cid = exports["isPed"]:isPed("cid")
--     TriggerServerEvent("varial-jobmanager:onCharSwap", cid)
--     Citizen.Wait(1000)   
...
function Apart.openHotelStash()
    ...
    local myjob = exports["isPed"]:isPed("myjob")
    if Apart.currentRoomLockDown[Apart.currentRoomType][Apart.currentRoomNumber] and (myjob ~= "police" or myjob ~= "state" or myjob ~= "sheriff" or myjob ~= "judge") then 
    ...
    if Apart.defaultInfo[1] == Apart.currentRoomType and Apart.defaultInfo[2] == Apart.currentRoomNumber then -- owner
        ...
        if Apart.currentRoomLockDown[Apart.currentRoomType][Apart.currentRoomNumber] and (myjob == "police" or  myjob == "judge" or myjob == "sheriff" or myjob == "state") then 
            Apart.OpenStash()
        end
    end
end
...
function garageToWorld()
    ...
    exports["varial-build"]:getModule("func").exitCurrentRoom(vector3(4.67, -724.85, 32.18))
    Apart.currentRoomNumber = Apart.defaultInfo[2]
    Apart.currentRoomType = Apart.defaultInfo[1]
    Apart.currentHotelInformation = nil
    Apart.currentGarageNumber = 0
...
function Apart.WakeFromBed()
    local bedOffset = exports["varial-build"]:getModule("func").getCurrentBed()
...
function nearClothing()
    ...
    local isNear = exports["varial-build"]:getModule("func").isNearCurrentInteract(1,5.5)
    if isNear then
        return true
    end
    ...
    if exports['raid_clothes']:isNearClothing() then
        return true
    end
...
AddEventHandler('apartment:removeFromBuilding', function(roomNumber,buildType)
    if Apart.currentRoomType == buildType and Apart.currentRoomNumber == roomNumber then
        if exports["varial-build"]:getModule("func").isInBuilding() then 
...
RegisterUICallback("varial-apartments:handler", function(data, cb)
...
AddEventHandler('apartments:menuAction', function(action)
    ...
    elseif action == "forfeit" then
        -- exports["varial-ui"]:showContextMenu(MenuData["apartment_check"])
    end
end)
