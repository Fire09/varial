    ...
    Citizen.CreateThread(function()
        ...
        while isPending do
            ...
            if count >= Config.Timeout and isPending then
                ...
                TriggerServerEvent('varial-ping:server:SendPingResult', pendingPing.id, 'timeout')
                pendingPing = nil
                isPending = false
            end
            Citizen.Wait(1000)
...
RegisterNetEvent('varial-ping:client:SendPing')
AddEventHandler('varial-ping:client:SendPing', function(sender, senderId, pCoords)
    if pendingPing == nil then
        ...
        TriggerServerEvent('varial-ping:server:SendPingResult', pendingPing.id, 'received')
        ...
        TriggerServerEvent('varial-ping:server:SendPingResult', senderId, 'unable')
    end
end)

RegisterNetEvent('varial-ping:client:AcceptPing')
AddEventHandler('varial-ping:client:AcceptPing', function()
    if isPending then
        ...
        TriggerServerEvent('varial-ping:server:SendPingResult', pendingPing.id, 'accept')
        isPending = false
    else
...
RegisterNetEvent('varial-ping:client:RejectPing')
AddEventHandler('varial-ping:client:RejectPing', function()
    if pendingPing ~= nil then
        TriggerEvent('DoLongHudText', 'Rejected Ping From ' .. pendingPing.name, 2)
        TriggerServerEvent('varial-ping:server:SendPingResult', pendingPing.id, 'reject')
        pendingPing = nil
        isPending = false
    else
        TriggerEvent('DoLongHudText', 'You Have No Pending Ping', 2)
...
RegisterNetEvent('varial-ping:client:RemovePing')
AddEventHandler('varial-ping:client:RemovePing', function()
    if pendingPing ~= nil then
        RemoveBlip(pendingPing.blip)
        pendingPing = nil
...
RegisterCommand('ping', function(src, args, raw)
    TriggerServerEvent("varial-ping:attempt",  GetEntityCoords(PlayerPedId()), args[1])
end)
