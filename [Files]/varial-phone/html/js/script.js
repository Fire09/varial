...
$(document).ready(function () {
    ...
    $.post('http://varial-phone/getWeather', JSON.stringify({}));
    ...
    document.onkeyup = function (data) {
        ...
        if (data.which == 27) {
            $.post('http://varial-phone/close', JSON.stringify({}));
        }
    }
        ...
        if (action !== undefined) {
            switch (action) {
                case "yellow-pages-delete":
                    $.post('http://varial-phone/deleteYP', JSON.stringify({}));
                    break;
                case "racing-create":
                ...
                case "newPostSubmit":
                    e.preventDefault();
                    $.post('http://varial-phone/newPostSubmit', JSON.stringify({
                    ...
                    break;
                case "group-manage":
                    $.post('http://varial-phone/manageGroup', JSON.stringify({ GroupID: $(this).data('action-data') }));
                    break;
                case "btnTaskGang":
                    manageGroup = $(this).data('action-data');
                    $.post('http://varial-phone/btnTaskGang', JSON.stringify({}));
                    break;
                ...
                case "getCallHistory":
                    if (callStates[currentCallState] === "isCallInProgress" && currentContainer !== "incoming-call")
                        ...
                        $.post('http://varial-phone/' + action, JSON.stringify({}));
                    break;
                case "spotify":
    ...
    window.addEventListener('message', function (event) {
        ...
        if (item.openPhone === true) {
            ...
            document.getElementById("tela").style.backgroundImage = "url(https://i.imgur.com/YxatSXR.jpeg&w=1366&h=768" +")";
            if (item.wallpaper) {
        ...
        switch (item.openSection) {
            ...
            case "newsms":
                ...
                break;
            case "phonemedio":
                $(".jss13").removeClass("slideout").addClass('slidein').show().css("bottom" , "-500px").add($('.jss13')).fadeIn();
                $(".phone-screen").removeClass("slideout").addClass('slidein').show().css("bottom" , "-481px").add($('.phone-screen')).fadeIn();
                $(".phone-app").removeClass("slideout").addClass('slidein').show().css("bottom" , "-500px").add($('.phone-screen')).fadeIn();
                break;
            ...
            case "racing:hud:update":
                switch (item.hudState) {
                    ...
                    case "finished":
                        ...
                        drawRaceStats();
                        $.post('http://varial-phone/race:completed', JSON.stringify({
...
function addRace(race, raceId) {
    ...
    `
    if (race.open)
        raceElement += `<button id="hovercorridas" style="margin-left: 140px;" class="racing-entries-join phone-button" data-id="${race.identifier}" aria-label="Join race" data-balloon-pos="up"><i class="fas fa-flag-checkered icon"></i></button> `
...
function openBrowser(url) {
    ...
    $.post('http://varial-phone/btnCamera', JSON.stringify({}));
    $("#browser").fadeIn(300);
}
...
function addMessage(item) {
    ...
    element.click(function () {
        $.post('http://varial-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: item.msgDisplayName }));
    });
...
function addMessageOther(item) {
    ...
    element.id = item.id;
    element.click(function () {
        $.post('http://varial-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: receiver, clientPhone: item.clientNumber }));
    });
    $(".messages-entries").prepend(element);
}
...
function updateSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            ...
            break;
        case "control":
            $.post('http://varial-phone/settingsUpdateToko', JSON.stringify({tag: "controlUpdate", controls: currentBinds}));
            break;
        case "browser":
            break;
        default:
...
function ResetSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            $.post('http://varial-phone/settingsResetToko', JSON.stringify());
            break;
        case "control":
            $.post('http://varial-phone/settingsResetControls', JSON.stringify());
            break;
        case "browser":
            break;
        default:
...
async function updateTokoSettings()
{
    ...
    await delayedLog();
    $.post('http://varial-phone/settingsUpdateToko', JSON.stringify({
        tag: "settings",
...
$('#racing-create-form').on('reset', function (e) {
    $.post('http://varial-phone/racing:map:cancel', JSON.stringify({}));
});

$('#racing-start-tracks').on('change', function (e) {
    let selectedMap = $(this).val();
    if(maps[selectedMap] !== undefined) {
        $.post('http://varial-phone/racing:map:removeBlips', JSON.stringify({}));
        $.post('http://varial-phone/racing:map:load', JSON.stringify({ id: selectedMap}));
...
$('#racing-start').submit(function (e) {
    ...
    $.post('http://varial-phone/racing:event:start', JSON.stringify({
...
$('#racing-create-form').submit(function (e) {
    e.preventDefault();
    $.post('http://varial-phone/racing:map:save', JSON.stringify({
...
$("#real-estate-sell-form").submit(function (e) {
    e.preventDefault();
    $.post('http://varial-phone/btnAttemptHouseSale', JSON.stringify({
...
$('#real-estate-transfer-form').submit(function (e) {
    e.preventDefault();
    $.post('http://varial-phone/btnTransferHouse', JSON.stringify({
...
$("#group-manage-pay-form").submit(function (e) {
    ...
    $.post('http://varial-phone/payGroup', JSON.stringify({
...
$("#group-manage-rank-form").submit(function (e) {
    e.preventDefault();
    $.post('http://varial-phone/promoteGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-manage-bank-form").submit(function (e) {
    ...
    $.post('http://varial-phone/bankGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-tasks-assign-modal-form").submit(function (e) {
    ...
    $.post('http://varial-phone/btnGiveTaskToPlayer', JSON.stringify({
...
$("#contacts-form").submit(function (e) {
    ...
    $.post('http://varial-phone/newContactSubmit', JSON.stringify({
        name: clean,
...
$("#wallpaper-form").submit(function (e) {
    ...
    $.post('http://varial-phone/updateMyWallpaper', JSON.stringify({
        name: escapedName,
    }));
...
$("#stock-form").submit(function (event) {
    event.preventDefault();
    $.post('http://varial-phone/stocksTradeToPlayer', JSON.stringify({
...
$("#twat-form").submit(function (event) {
    event.preventDefault();
    $.post('http://varial-phone/newTwatSubmit', JSON.stringify({
...
$("#call-form").submit(function (event) {
    event.preventDefault();
    $.post('http://varial-phone/callContact', JSON.stringify({
        name: '',
...
$("#new-message-form").submit(function (event) {
    ...
    $.post('http://varial-phone/newMessageSubmit', JSON.stringify({
    ...
    switch (currentContainer) {
        case "message":
            setTimeout(function () {
                ...
                $.post('http://varial-phone/messageRead', JSON.stringify({ sender: sender, receiver: receiver, displayName: displayName }));
            }, 300);
            break;
        case "messages":
            setTimeout(function () {
                $.post('http://varial-phone/messages', JSON.stringify({}));
            }, 300);
            break;
    }
...
$('.racing-map-delete-confirm').click(function () {  
    ...
    $.post('http://varial-phone/racing:map:delete', JSON.stringify({ id: raceMap }));
    $('.racing-delete-confirm').fadeOut(150)
...
$('#real-estate-evict-modal-accept').click(function () {
    $.post('http://varial-phone/btnEvictHouse', JSON.stringify({}));
...
$('.twatter-toggle-notification').click(function () {
    ...
    $.post('http://varial-phone/btnNotifyToggle', JSON.stringify({}));
});



$('.account-information-toggle-pager').click(function () {
    $.post('http://varial-phone/btnPagerToggle', JSON.stringify({}));
...
$('.racing-entries').on('click', '.racing-entries-join', function () {
    $.post('http://varial-phone/racing:event:join', JSON.stringify({ identifier: $(this).data('id') }));
});

$('.keys-entries').on('click', '.manage-keys', function () {
    $.post('http://varial-phone/retrieveHouseKeys', JSON.stringify({}));
});

$('.keys-entries').on('click', '.remove-shared-key', function(e) {
    $.post('http://varial-phone/removeSharedKey', JSON.stringify({
...
$('.manage-keys-entries').on('click', '.manage-keys-remove', function () {
    $.post('http://varial-phone/removeHouseKey', JSON.stringify({
...
$('.group-tasks-entries').on('click', '.group-tasks-track', function () {
    $.post('http://varial-phone/trackTaskLocation', JSON.stringify({ TaskIdentifier: $(this).data('id') }));
});

$('.delivery-job-entries').on('click', '.delivery-job-accept', function (e) {
    $.post('http://varial-phone/selectedJob', JSON.stringify({ jobType: $(this).data('job-type'), jobId: $(this).data('job-id') }));
});
...
$('.garage-container-entries-wrapper').on('click', '.garage-spawn', function (e) {
    e.preventDefault();
    $.post('http://varial-phone/vehspawn', JSON.stringify({ vehplate: $(this).data('plate') }));
    $.post('http://varial-phone/btnGarage', JSON.stringify({}));
});

$('.garage-container-entries-wrapper').on('click', '.garage-track', function () {
    $.post('http://varial-phone/vehtrack', JSON.stringify({ vehplate: $(this).data('plate') }));
});

$('.garage-entries').on('click', '.garage-pay', function (e) {
    $.post('http://varial-phone/vehiclePay', JSON.stringify({ vehiclePlate: $(this).data('plate') }));
    setTimeout(function () {
        $.post('http://varial-phone/btnGarage', JSON.stringify({}));
    }, 1500);
});
...
$('.gps-entries, .keys-entries').on('click', '.gps-location-click', function () {
    $.post('http://varial-phone/loadUserGPS', JSON.stringify({ house_id: $(this).data('house-id'), house_type: $(this).data('house-type') }));
})

$('.contacts-entries-wrapper, .call-history-entries-wrapper').on('click', '.btn-contacts-call', function () {
    $.post('http://varial-phone/callContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
});
...
$('#confirm-modal-accept').click(function (event) {
    $.post('http://varial-phone/removeContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
...
function openPhoneShell() {
    ...
    $(".jss13").removeClass("closephone").addClass('openphone').show().css("bottom" , "12px").add($('.jss13')).fadeIn(500);
   
}

function closePhoneShell() {
    $(".jss13").removeClass("openphone").addClass("closephone").fadeOut()
...
$("#twat-gifword").change(function() {
   ...
   $(document).on('click', '#twat-sendimg', function (e) {
    ...
    $.post('http://varial-phone/phone:selfie', JSON.stringify({}));
    e.preventDefault();
    $.post('http://varial-phone/PostNewImage', JSON.stringify({}),
        function (url) {
...
function SetWaypointPostOP() {
    $.post('http://varial-phone/SetPostOpNewWaypoint', JSON.stringify({}));
}

function SetWaypointFarming() {
    $.post('http://varial-phone/SetFarmerJobWaypoint', JSON.stringify({}));
}

function SetWaypointFarmingSales() {
    $.post('http://varial-phone/SetFarmerJobWaypointSale', JSON.stringify({}));
}

function SetWaypointFarmingProcess() {
    $.post('http://varial-phone/SetFarmerJobWaypointProcess', JSON.stringify({}));
}

function SetWaypointMining() {
    $.post('http://varial-phone/SetMiningNewWaypoint', JSON.stringify({}));
}

function SetWaypointWaterNPower() {
    $.post('http://varial-phone/SetWaterNPowerNewWaypoint', JSON.stringify({}));
}

function SetWayPointGarbage() {
    $.post('http://varial-phone/SetGarbageNewWaypoint', JSON.stringify({}));
}

function SetWayPointChickenJob() {
    $.post('http://varial-phone/SetChickenNewWaypoint', JSON.stringify({}));
}

function SetWayPointFishingJob() {
    $.post('http://varial-phone/SetFishingNewWaypoint', JSON.stringify({}));
}

function SetWayPointHunting() {
    $.post('http://varial-phone/SetHuntingNewWaypoint', JSON.stringify({}));
}

// Sale Locations

function MiningSales() {
    $.post('http://varial-phone/SetMiningSalesLocation', JSON.stringify({}));
}

function ChickenSales() {
    $.post('http://varial-phone/SetChickenSalesLocation', JSON.stringify({}));
}

function FishingSales() {
    $.post('http://varial-phone/SetFishingSalesLocation', JSON.stringify({}));
}

function HuntingSales() {
    $.post('http://varial-phone/SetHuntingSalesLocation', JSON.stringify({}));
}