...
if dist > 40.0 then
    TriggerEvent('phone:addJobNotify', 'From the PIN:<br> You are too far away.')
    TriggerEvent('DoLongHudText', 'I changed your GPS with the starting Line.', 2)
    EndRace()
    SetNewWaypoint(mapcheckPoints[checkPointIndex]["x"], mapcheckPoints[checkPointIndex]["y"], mapcheckPoints[checkPointIndex]["z"])
  return
end
...
RegisterNUICallback('racing:events:list', function()
  ...
  if exports["varial-inventory"]:hasEnoughOfItem("racingusb1",1,false) then
  SendNUIMessage({
