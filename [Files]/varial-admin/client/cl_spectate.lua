...
local function MessageShit (pMsg,type)
    TriggerEvent('DoLongHudText', pMsg,type)
  end
...
function toggleSpectate(targetPed, targetPlayerId)
    ...
    if isSpectateEnabled then
        ...
        clearGamerTagInfo()
        DoScreenFadeIn(500)
        MessageShit('Stopped spectating '..GetPlayerName(storedTargetPlayerId)..' ('..GetPlayerServerId(storedTargetPlayerId)..')')
        storedTargetPed = nil
    else
        ...
        DoScreenFadeIn(500)
        MessageShit('Started spectating '..GetPlayerName(storedTargetPlayerId)..' ('..GetPlayerServerId(storedTargetPlayerId)..')')
...
function cleanupFailedResolve()
    ...
    MessageShit('Stopped spectating, could not find player..', 'error')
end
...
RegisterNetEvent('Mercy/client/specPlayer', function(targetServerId, coords)
    ...
    if targetPlayerId == PlayerId() then
        return MessageShit('You can\'t spectate yourself..', 'error')
    end
