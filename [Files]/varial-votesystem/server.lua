...
function gangCoords(gangType,src)
  ...
  exports.oxmysql:execute("SELECT coords FROM " .. gangtable .. "HERE id = @id", {['id'] = 1}, function(result)
...
function CompleteUpdate (currentFunds)
  ...
  exports.oxmysql:execute("UPDATE general_variables SET value = @value WHERE id = @name", {['value'] = currentFunds, ['name'] = 3})
end

RegisterServerEvent('updatecoordsweed')
AddEventHandler('updatecoordsweed', function(gangType,newCoords)
  exports.oxmysql:execute("UPDATE weed_farming SET coords = @newCoords WHERE id = @id", {['newCoords'] = newCoords,['id'] = 1})
...
function updateCoords(gangType,newCoords)
  ...
  exports.oxmysql:execute("UPDATE" .. gangtable .. " SET coords = @newCoords WHERE id = @id",{['newCoords'] = newCoords,['id'] = 1})
  TriggerClientEvent("gang:setcoords",-1,newCoords,gangType)

end

function updateMayorId()
  exports.oxmysql:execute("SELECT value FROM general_variables WHERE id = @id", {['id'] = 2}, function(result)
    mayorid = result[1].value
  end)
end

function updateCurrentTax()
  exports.oxmysql:execute("SELECT value FROM general_variables WHERE id = @id", {['id'] = 4}, function(result)
...
function AddFunds()
  if FundsToAdd ~= 0 then
    ...
    exports.oxmysql:execute("SELECT value FROM general_variables WHERE id = @id", {['id'] = 3}, function(result)
...
passnames = {
  ...
  ['bestbuds'] = "Best Buds",
  ['vu'] = "Vanilla Unicorn",
  ['red_circle'] = "Red Circle Nightclub",
  ['pdm'] = "PDM",
  ['bondi_boys'] = "Bondi Boys",
  ['lostmc'] = "LostMC",
  ['the_family'] = "The Family",
  ['illegal_shop'] = "Illegal Shop",
  ['corpsemen'] = "The Corpsemen",
  ['flight_school'] = "Flight School",
  ['ugcasino'] = "Underground Casino",
}

-- here we wipe the weed group and start over.

RegisterServerEvent('server:givepayJob')
AddEventHandler('server:givepayJob', function(pJob, pAmount)
  local src = source
  local user = exports["varial-base"]:getModule("Player"):GetUser(src)
  local char = user:getCurrentCharacter()
  local cid = tonumber(char.id)
  exports.oxmysql:execute("SELECT id, paycheck FROM characters WHERE id = @id", {['id'] = cid}, function(result)
    if result[1] ~= nil then
  ...
  exports.oxmysql:execute("SELECT id,paycheck FROM characters WHERE id = @id", {['id'] = cid}, function(result)
    if result[1] ~= nil then
      ...
      if curPaycheck > 0 then
        exports.oxmysql:execute("UPDATE characters SET paycheck = 0 WHERE id  = @id", { ['id'] = cid})
...
AddEventHandler('server:givepass', function(pass_type, wrank, cid)
  ...
  local user = exports["varial-base"]:getModule("Player"):GetUser(src)
  local char = user:getCurrentCharacter()
  exports.oxmysql:execute("SELECT rank FROM character_passes WHERE cid = @id", {['id'] = cid}, function(result)
    if result[1].rank >= 4 then
      exports.oxmysql:execute("UPDATE character_passes SET `rank` = @rank, `giver` = @giver WHERE cid  = @id", { ['id'] = cid, ['rank'] = wrank, ['giver'] = char.first_name .. ' ' .. char.last_name})
    else
      -- TriggerClientEvent('DoShortHudText', src, 'Person is not in your group', 2)
      exports.oxmysql:execute("INSERT INTO character_passes(cid, rank, name, giver, pass_type, business_name) values (@id, @rank, @name, @giver, @gang_id, @business_name)",
      {
          ['id'] = cid,
          ['rank'] = wrank,
...
AddEventHandler('server:givepayGroup', function(groupname, amount, cid)
  ...
  local user = exports["varial-base"]:getModule("Player"):GetUser(src)
  local char = user:getCurrentCharacter()
  exports.oxmysql:execute("SELECT bank FROM group_banking WHERE group_type = @id", {['id'] = groupname}, function(data)
    if data[1].bank >= tonumber(amount) then
      exports.oxmysql:execute("SELECT paycheck FROM characters WHERE id = @id", {['id'] = cid}, function(result)
          ...
          exports.oxmysql:execute("UPDATE characters SET `paycheck` = @paycheck WHERE id = @id", { ['id'] = cid, ['paycheck'] = solution})
          exports.oxmysql:execute("UPDATE group_banking SET `bank` = @bank WHERE group_type = @group_type", { ['group_type'] = groupname, ['bank'] = solution2})
        end)
    else
...
AddEventHandler('server:gankGroup', function(groupid, amount)
  ...
  if user:getCash() >= tonumber(amount) then
    user:removeMoney(tonumber(amount))
      exports.oxmysql:execute("SELECT bank FROM group_banking WHERE group_type = @id", {['id'] = groupid}, function(result)
        local solution = result[1].bank + amount
        exports.oxmysql:execute("UPDATE group_banking SET `bank` = @bank WHERE `group_type` = @id", { ['id'] = groupid, ['bank'] = solution})
      end)
    else
...
AddEventHandler('setPolicePay', function(newPay)
  ...
  exports.oxmysql:execute("UPDATE general_variables SET value = @value WHERE id = @name",{['value'] = newPay, ['name'] = 7})
...
AddEventHandler('checkmayorname', function()
  ...
  exports.oxmysql:execute("SELECT first_name, last_name FROM characters WHERE id = @id", {['id'] = mayorid}, function(result)
...
AddEventHandler('server:PayStoreOwner', function(cid,cashamount)
  ...
  exports.oxmysql:execute("SELECT id,paycheck FROM characters WHERE id = @cid", {['cid'] = cid}, function(result)
    if result[1] ~= nil then
      local curPaycheck = result[1].paycheck
      local taxWithheld = (cashamount * (mayortax / 100))
      curPaycheck = curPaycheck + (cashamount - (taxWithheld))
      exports.oxmysql:execute("UPDATE characters SET paycheck = @curPayCheck WHERE id = @cid",{['cid'] = cid,['curPayCheck'] = curPaycheck})
