...
RegisterServerEvent('varial-hospital:server:RequestBed')
AddEventHandler('varial-hospital:server:RequestBed', function()
    for k, v in pairs(beds) do
        if not v.taken then
            v.taken = true
            bedsTaken[source] = k
            TriggerClientEvent('varial-hospital:client:SendToBed', source, k, v)
            return
        end
    end
...
RegisterServerEvent('varial-hospital:server:RPRequestBed')
AddEventHandler('varial-hospital:server:RPRequestBed', function(plyCoords)
    ...
    for k, v in pairs(beds) do
        ...
        if distance < 3.0 then
            if not v.taken then
                v.taken = true
                foundbed = true
                TriggerClientEvent('varial-hospital:client:RPSendToBed', source, k, v)
                return
            else
...
RegisterServerEvent('varial-hospital:server:EnteredBed')
AddEventHandler('varial-hospital:server:EnteredBed', function()
    local src = source
    ...
    TriggerClientEvent('varial-hospital:client:FinishServices', src)
end)

RegisterServerEvent('varial-hospital:server:LeaveBed')
AddEventHandler('varial-hospital:server:LeaveBed', function(id)
    beds[id].taken = false
end)