...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('varial-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('varial-hospital:client:SyncBleed')
AddEventHandler('varial-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('varial-hospital:client:FieldTreatLimbs')
AddEventHandler('varial-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('varial-hospital:client:FieldTreatBleed')
AddEventHandler('varial-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('varial-hospital:client:ReduceBleed')
AddEventHandler('varial-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('varial-hospital:client:ResetLimbs')
AddEventHandler('varial-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('varial-hospital:client:RemoveBleed')
AddEventHandler('varial-hospital:client:RemoveBleed', function()
    isBleeding = 0
