...
RegisterNetEvent("varial-hospitalization:checkin")
AddEventHandler("varial-hospitalization:checkin", function()
	if exports["isPed"]:isPed("countems") <= 0 or exports["isPed"]:isPed("myJob") == 'ems'  then
		...
		local finished = exports["varial-taskbar"]:taskBar(1700,"Checking Credentials")
...
RegisterNetEvent("varial-hospitalization:page")
AddEventHandler("varial-hospitalization:page", function()
	loadAnimDict('anim@narcotics@trash')
	TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',1.0, 1.0, -1, 1, 0, 0, 0, 0)
	local finished = exports["varial-taskbar"]:taskBar(1700,"Paging a Doctor")
	if finished == 100 then
...
AddEventHandler("client:bed",function()
  ...
  if DoesEntityExist(objFound) then
	...
	exports['varial-interaction']:showInteraction("[E] Leave Bed")
...
function findBed(fadein)
	...
	if not inbed then
		...
		TriggerServerEvent("varial-death:reviveSV2")
	end
end
