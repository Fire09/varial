...
RegisterCommand('unjail', function(source, args)
    ...
    local user = exports["varial-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local target = exports["varial-base"]:getModule("Player"):GetUser(args[1])
    ...
    if user:getVar("job") == 'police' or user:getVar("job") == 'state' or user:getVar("job") == 'police' or user:getVar("job") == 'doc' or user:getVar("job") == 'judge' then
        if args[1] and exports["varial-base"]:getModule("Player"):GetUser(tonumber(args[1])) then
...
RegisterServerEvent('varial-jail:remove-time-electrical')
AddEventHandler("varial-jail:remove-time-electrical", function()
    local src = source
    local user = exports["varial-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    exports.oxmysql:execute("SELECT * FROM `characters` WHERE id = @cid", {['cid'] = char.id}, function(data)
        ...
        exports.oxmysql:execute("UPDATE `characters` SET `jail_time` = @time WHERE `id` = @cid", {['time'] = recent - tonumber(math.random(2, 8)), ['cid'] = char.id})
    end)
end)
