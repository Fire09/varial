		...
		Citizen.Wait(1)
			if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 10.0)then
				...
				if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 5.0)then
					...
					local finished = exports["varial-taskbar"]:taskBar(60000,"Searching...")
...
RegisterNetEvent('varial-shops:exchangeGold')
AddEventHandler('varial-shops:exchangeGold', function()
	if exports["varial-inventory"]:hasEnoughOfItem("goldbar",70,true) then
		local finished = exports["varial-taskbar"]:taskBar(120000,"Trading Bars")
		if finished == 100 then
			if exports["varial-inventory"]:hasEnoughOfItem("goldbar",70,true) then
				TriggerServerEvent('varial-banking:addMoney', 35000)
...
AddEventHandler('slushy:general', function()
	local finished = exports["varial-taskbar"]:taskBar(60000,"Making a god slushy...")
	if (finished == 100) then
...
AddEventHandler('lockpickshit:general', function()
	local finished = exports["varial-taskbar"]:taskBar(60000,"What dis????")
	if (finished == 100) then
...
AddEventHandler('smelt:crafting', function()
	local finished = exports['varial-taskbar']:taskBar(20000, 'Smelting')
	if (finished == 100) then
...
RegisterNetEvent('autos:crafting')
AddEventHandler('autos:crafting', function()
    local job = exports["isPed"]:GroupRank('auto_exotics')
    if job >= 3 then
		TriggerEvent("server-inventory-open", "27", "Craft");	
		Wait(1000)
	else
		TriggerEvent("DoLongHudText", "You are not a worker here!", 2)
    end
end)

RegisterNetEvent('ottos:crafting')
AddEventHandler('ottos:crafting', function()
    local job = exports["isPed"]:GroupRank('ottos_autos')
    if job >= 3 then
		TriggerEvent("server-inventory-open", "27", "Craft");	
		Wait(1000)
	else
...
RegisterNetEvent('tunershop:crafting')
AddEventHandler('tunershop:crafting', function()
    local job = exports["isPed"]:GroupRank('tuner_shop')
    if job >= 3 then
		TriggerEvent("server-inventory-open", "27", "Craft");	
		Wait(1000)
...
RegisterNetEvent('ak_customs:crafting')
AddEventHandler('ak_customs:crafting', function()
    local job = exports["isPed"]:GroupRank('ak_customs')
    if job >= 3 then
		TriggerEvent("server-inventory-open", "27", "Craft");	
		Wait(1000)
...
RegisterNetEvent("varial-shops:redCounter")
AddEventHandler("varial-shops:redCounter", function()
    TriggerEvent("server-inventory-open", "1", "redcircle_counter");
end)

RegisterNetEvent("varial-shops:artGallery1")
AddEventHandler("varial-shops:artGallery1", function()
    TriggerEvent("server-inventory-open", "1", "gallery_1");
end)

RegisterNetEvent("varial-shops:artGallery2")
AddEventHandler("varial-shops:artGallery2", function()
    TriggerEvent("server-inventory-open", "1", "gallery_2");
end)

RegisterNetEvent("varial-shops:redCounter2")
AddEventHandler("varial-shops:redCounter2", function()
...
AddEventHandler('weapon:general', function()
	TriggerServerEvent('varial-shops:WeaponShopStatus')
end)
...
AddEventHandler("hunting:level1", function()
	if exports["varial-inventory"]:getQuantity("coyotepelt") >= 1 then
		playerAnim()
		local finished = exports["varial-taskbar"]:taskBar(4000,"Selling Coyotepelt",true,false,playerVeh)
		if finished == 100 then
			if exports["varial-inventory"]:getQuantity("coyotepelt") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'coyotepelt', 1)
				TriggerServerEvent('varial-banking:addMoney', math.random(80, 150))
			else
...
AddEventHandler("hunting:level2", function()
	if exports["varial-inventory"]:getQuantity("buck") >= 1 then
		playerAnim()
		local finished = exports["varial-taskbar"]:taskBar(4000,"Selling Buck",true,false,playerVeh)
		if finished == 100 then
			if exports["varial-inventory"]:getQuantity("buck") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'buck', 1)
				TriggerServerEvent('varial-banking:addMoney', math.random(160, 200))
			else
...
AddEventHandler("hunting:level3", function()
	if exports["varial-inventory"]:getQuantity("boar") >= 1 then
		playerAnim()
		local finished = exports["varial-taskbar"]:taskBar(4000,"Selling Boar",true,false,playerVeh)
		if finished == 100 then
			if exports["varial-inventory"]:getQuantity("boar") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'boar', 1)
				TriggerServerEvent('varial-banking:addMoney', math.random(350, 500))
			else
...
--     exports['varial-menuinteract']:AddButton("Sell Coyotepelt", "Wow you suck", "hunting:level1", 'example button 1', "jobmenu")
--     exports['varial-menuinteract']:AddButton("Sell Buck", "Wow you kinda okay", "hunting:level2", 'example button 2', "jobmenu")
--     exports['varial-menuinteract']:AddButton("Sell Boar", "Wow you good", "hunting:level3", 'example button 3', "jobmenu")
--     exports['varial-menuinteract']:SubMenu("Hunting Menu" , "Click to select sell!" , "jobmenu")
-- end

RegisterNetEvent("varial-scoreboard:playerscount")
AddEventHandler("varial-scoreboard:playerscount", function(a)
    SetRichPresence(a .. "/64 | Come Join Varial TODAY!")
    SetDiscordAppId(979775386474922015)
	SetDiscordRichPresenceAsset("a Server")
	-- SetDiscordRichPresenceAssetSmall("green")
	SetDiscordRichPresenceAssetText('A server soonTM')
    SetDiscordRichPresenceAction(0, 'Join Today!', '')
end)
...
RegisterNetEvent('craft:weapons')
AddEventHandler('craft:weapons', function()
