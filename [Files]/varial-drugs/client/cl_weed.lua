...
RegisterNetEvent('varial-weed:knownCrops')
AddEventHandler('varial-weed:knownCrops', function(sentInfo, currentTime)
...
RegisterNetEvent('varial-weed:plantSeed')
AddEventHandler('varial-weed:plantSeed', function(sentType)
    if not isBusy then
        if exports["varial-inventory"]:hasEnoughOfItem("plantpot",1,false) then
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if shouldBreak == true then
                ...
                DeleteEntity(CreatedObject)
                local finished = exports["varial-taskbar"]:taskBar(math.random(8000, 12000),"Planting",false,false)
                if (finished == 100) then
                    if exports["varial-inventory"]:hasEnoughOfItem(sentType,1,false) then
...
RegisterNetEvent('varial-weed:destroyPlant')
AddEventHandler('varial-weed:destroyPlant', function(coords)
    if not isBusy then
        isBusy = true
                ...
                CreateThread(function()
                    ...
                    while true do
                        ...
                        if GetGameTimer() >= endTime then
                            TriggerServerEvent("varial-weed:killplant", knownCrops[knownInfo['id']]['knownId'])
                            break
                        end
                    end
                end)
...
function NewPlant(sentType, coordsX, coordsY, coordsZ)
	TriggerServerEvent("varial-weed:newplant", coordsX, coordsY, coordsZ, sentType)
end 

RegisterNetEvent('varial-weed:updatePlant')
AddEventHandler('varial-weed:updatePlant', function(updateType, sentId, sentGrowth, sentTime, sentStatus, sentNewInfo, currentTime)
            ...
            if cropDist < 1.8 then
                ...
                if cropDist < 2 then
                    if IsControlJustReleased(0, 304) then
                        if knownCrops[closest]["growth"] >= 100 then
                            local finished = exports["varial-taskbar"]:taskBar(1000, "Picking Plant", false, false)
                            if (finished == 100) then
                                TriggerServerEvent('varial-weed:killplant', knownCrops[closest]["knownId"])
                        ...
                        elseif num == 2 then
                            if exports["varial-inventory"]:hasEnoughOfItem("purifiedwater",1,false) then
                                TriggerEvent("inventory:removeItem", "purifiedwater", 1)
                                local new = math.ceil(knownCrops[closest]["growth"] + math.random(10,15))
                                TriggerServerEvent("varial-weed:updateStatus", 'update', knownCrops[closest]["knownId"], new, 'water')
                            else
                                TriggerEvent("DoLongHudText", "You need purified water.", 2)
                            end
               
                        elseif num == 3 then
                            if exports["varial-inventory"]:hasEnoughOfItem("fertilizer",1,false) then
                                TriggerEvent("inventory:removeItem", "fertilizer", 1)
                                local new = math.ceil(knownCrops[closest]["growth"] + math.random(10,15))
                                TriggerServerEvent("varial-weed:updateStatus", 'update', knownCrops[closest]["knownId"], new, 'fertilizer')
                            else
