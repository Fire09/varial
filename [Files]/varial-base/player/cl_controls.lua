Void.DataControls = Void.DataControls or {}
Void.Controls = Void.Controls or {}
Void.Controls.EventHolder = {}
...
Void.Controls.secondaryBinds = {}

Void.Controls.Distcheck = {}
Void.Controls.Distcheck.use = {
	-- create fake id
...
Void.Controls.Distcheck.useSecondary = {
...
Void.Controls.Distcheck.useThird = {
	-- hospital garage
...
function Void.DataControls.getBindTable()

	local i = 1
	local controlTable = {}
	for k,v in pairs(Void.Controls.Current) do
		controlTable[i] = {k,v}
		i = i+1
	end
...
function Void.DataControls.encodeSetBindTable(self, bindTable)
	...
	Void.DataControls.setBindTable(controlTable,true)
end

function Void.DataControls.toUpper(table)
...
function Void.DataControls.setBindTable(controlTable,shouldSend)
	if controlTable == nil then
		Void.Controls.Current  = Void.DataControls.toUpper(Void.Controls.Default) 
		Void.DataControls.setSecondaryBindTable(Void.Controls.Current)
		TriggerServerEvent('varial-base:sv:player_control_set',Void.Controls.Current)
		Void.DataControls.checkForMissing()
	else
		if shouldSend then 
			Void.Controls.Current = Void.DataControls.toUpper(controlTable)
			Void.DataControls.setSecondaryBindTable(Void.Controls.Current)
			TriggerServerEvent('varial-base:sv:player_control_set',Void.Controls.Current)
			Void.DataControls.checkForMissing()
		else
	...
	TriggerEvent("event:control:update",Void.DataControls.getTableInKeyNumbers())
end

function Void.DataControls.setSecondaryBindTable(bindtable)
	...
	for k,v in pairs(bindtable) do
		...
		if string.match(keyString, "+")  then
			...
			local valid = true
			for i=1,#Void.Controls.secondaryBinds  do
				if Void.Controls.secondaryBinds[i] and Void.Controls.secondaryBinds[i][1] == control1 then valid = false end
			end

			if valid then 
				Void.Controls.secondaryBinds[i] = {}
				Void.Controls.secondaryBinds[i][1] = control1
				Void.Controls.secondaryBinds[i][2] = control2
				i = i + 1
			end
...
function Void.DataControls.getTableInKeyNumbers()
...
function Void.DataControls.getBindByID(bindID)
	local found = "none"
	for k,v in pairs(Void.Controls.Current) do
		if k == bindID then
			found = v
...
function Void.DataControls.distanceCall(distcheckName)
	local distanceTable = Void.Controls.Distcheck[distcheckName]
	local found = -1
	...
	if found ~= -1 then
		...
		if Void.Controls.EventHolder[key] then
			hasTimer = (GetGameTimer()-Void.Controls.EventHolder[key])
		end
		if  hasTimer >= distanceTable[found].maxwait then
			Void.Controls.EventHolder[key] = GetGameTimer();
...
function Void.DataControls.checkForMissing()
	local isMissing = false

	for j,h in pairs(Void.Controls.Default) do
		if Void.Controls.Current[j] == nil then
			isMissing = true
			Void.Controls.Current[j] = h
		end
	end
...
function Void.DataControls.validEvent(event,id,eventID)
	local v = Void.Controls.events["general"][eventID]
	local hasTimer = 999999
	local eventKey = event.."_"..id

	if Void.Controls.EventHolder[eventKey] then
		hasTimer = (GetGameTimer()-Void.Controls.EventHolder[eventKey])
	end

	if Void.Controls.EventHolder[eventKey] and hasTimer ~= 999999 then
		if hasTimer >= v.maxwait then
			Void.Controls.EventHolder[eventKey] = GetGameTimer();
...
Citizen.CreateThread(function()
	while true do
		...
		if Void.Controls.Current["tokoptt"] then
			for i=0,#Void.Controls.events["general"] do
				local v = Void.Controls.events["general"][i]
				local keyString = Void.Controls.Current[v["bind"]]
				local key = Keys[keyString]

				if key == 199 then
					if IsDisabledControlJustReleased(1,key) then
						local isConflicted = -1
						for i=0,#Void.Controls.secondaryBinds do
							if IsControlPressed(1,Void.Controls.secondaryBinds[i][1]) or IsDisabledControlPressed(1,Void.Controls.secondaryBinds[i][1]) then
								isConflicted = Void.Controls.secondaryBinds[i][1]
							end
						end
						...
						if isConflicted ~= -1 then
							...
							if IsControlPressed(1,isConflicted) or IsDisabledControlPressed(1,isConflicted) then
								...
								Void.DataControls.validEvent(eventBind,idEvent,i)
							else
								Void.DataControls.validEvent(v.event,v.id,i)
							end
						else
					...
					if IsControlJustPressed(1,key) and not IsControlJustPressed(1,0) and not IsControlJustPressed(1,nil) then
						if GetLastInputMethod(0) then
							...
							if validTrigger then
								local isConflicted = -1
								for i=0,#Void.Controls.secondaryBinds do
									if IsControlPressed(1,Void.Controls.secondaryBinds[i][2]) or IsDisabledControlPressed(1,Void.Controls.secondaryBinds[i][2]) then
										isConflicted = Void.Controls.secondaryBinds[i][2]
									end
								end
...
Citizen.CreateThread(function()
	while true do
		...
		if Void.Controls.Current["tokoptt"] then
			for i=0,#Void.Controls.events["general-dist"] do
				local v = Void.Controls.events["general-dist"][i]
				local key = Keys[Void.Controls.Current[v["bind"]]]
				local hasTimer = 99999;
				...
				if IsControlJustPressed(1,key) and hasTimer > 500 then	
					...
					Void.DataControls.distanceCall(v["distanceName"])
				end	
			end
		end
		Wait(5)
...
RegisterNetEvent("varial-base:cl:player_control")
AddEventHandler("varial-base:cl:player_control", function(controlTable)
	Void.DataControls.setBindTable(controlTable,false)
end)
...
RegisterNetEvent('varial-base:setcontrols')
AddEventHandler('varial-base:setcontrols', function()
	TriggerServerEvent('varial-base:sv:player_controls')
end)
