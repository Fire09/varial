...
-- RegisterServerEvent('varial-dispatch:GrabRadioChannel')
-- AddEventHandler("varial-dispatch:GrabRadioChannel", function(radio)
-- 	local src = source
...
RegisterServerEvent('varial-mdt:OpenMDT')
AddEventHandler("varial-mdt:OpenMDT", function()
	local src = source
	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
	...
	if userjob == "police" or userjob == "sheriff" or userjob == "state" or userjob == "ems" or userjob == "judge" or userjob == "district_attorney" or userjob == "public_defender" then
        TriggerClientEvent('varial-mdt:open', src, userjob, char.last_name, char.first_name)
        TriggerClientEvent('varial-voice:getRadioChannel', src)
	end
end)

-- RegisterServerEvent("varial-mdt:opendashboard")
-- AddEventHandler("varial-mdt:opendashboard", function()
--     local src = source
-- 	UpdateUnits(src)	
-- end) 
...
LoadEvanVersion = function()
    GetUserByCid = function(cid)
        for index, value in pairs(GetPlayers()) do
            local Player = exports["varial-base"]:getModule("Player"):GetUser(tonumber(value))
    ...
    GetCharData = function(source)
        local Player = exports["varial-base"]:getModule("Player"):GetUser(source)
        local char = Player:getCurrentCharacter()
    ...
    RPC.register("varial-mdt:getInfo", function()
        return {firstname = GetCharData(source).first_name, lastname = GetCharData(source).last_name}
    end)
    RPC.register("varial-mdt:dashboardbulletin", function()
        return GetAllBulletinData()
    end)
     RPC.register("varial-mdt:dashboardMessages", function()
         return GetAllMessages()
     end)
    RPC.register("varial-mdt:getWarrants", function()
        local WarrantData = {}
     ...
     function UpdateUnits(src)	
         ...
         TriggerClientEvent("varial-mdt:getActiveUnits", src, lspd, bcso, sasp, ems)
     end	    
    ...
    RPC.register("varial-mdt:getProfileData", function(identifier)
        local ReturnData = {}
        local result = SQL('SELECT * FROM characters WHERE id = @id', {['@id'] = identifier})
        ...
        ReturnData.Workplace = result[1].job
    ...
    RPC.register("varial-mdt:JailPlayer", function(cid, time)
        JailPlayer(source, cid, time)
    end)
    RPC.register("varial-mdt:saveProfile", function(profilepic, information, cid)
    ...
    RPC.register("varial-mdt:addGalleryImg", function(cid, url)
    ...
    RPC.register("varial-mdt:addWeapon", function(cid, serialnumber)
    ...
    RPC.register("varial-mdt:removeGalleryImg", function(cid, url)
        SQL("SELECT * FROM `characters` WHERE id = @id", {
            ['@id'] = cid
        }, function(result)
            local NewGallery = {}
    ...
    RPC.register("varial-mdt:newTag", function(identifier, tag)
        SQL("SELECT * FROM `characters` WHERE id = @id", {
            ['@id'] = identifier
        }, function(result)
            local NewTags = {}
    ...
    RPC.register("varial-mdt:removeProfileTag", function(cid, tagtext)
        SQL("SELECT * FROM `characters` WHERE id = @id", {
            ['@id'] = cid
        }, function(result)
            local NewTags = {}
    ...
    RPC.register("varial-mdt:updateLicence", function(type, status, cid)
        local src = source
        local user = exports["varial-base"]:getModule("Player"):GetUser(src)	
        local char = user:getCurrentCharacter()
    ...
    RPC.register("varial-mdt:deleteBulletin", function(id, time)
        local src = source
        local Player = exports["varial-base"]:getModule("Player"):GetUser(src)
    ...
    RPC.register("varial-mdt:newBulletin", function(title, info, time)
        local src = source
        local Player = exports["varial-base"]:getModule("Player"):GetUser(src)
    ...
    RPC.register("varial-mdt:searchProfile", function(name)
        local Matches = {}
    ...
    RPC.register("varial-mdt:searchIncidents", function(incident)
        local Matches = {}
    ...
    RPC.register("varial-mdt:saveIncident", function(ID, title, information, tags, officers, civilians, evidence, associated, time)
        local src = source
    ...
    RPC.register("varial-mdt:getAllIncidents", function()
        local Tables = {}
    ...
    RPC.register("varial-mdt:getIncidentData", function(id)
        local result = SQL('SELECT * FROM mdt_incidents WHERE id = @id', {['@id'] = id})
        local convictions = {}
    ...
    RPC.register("varial-mdt:incidentSearchPerson", function(name)
        local Matches = {}
        local users = SQL("SELECT * FROM characters", {})
        for index = 1, #users, 1 do
            local firstname = string.lower(users[index].first_name)
    ...
    RPC.register("varial-mdt:removeIncidentCriminal", function(cid, incidentId)
    ...
    RPC.register("varial-mdt:getPenalCode", function()
        return Config['OffensesTitels'], Config["Offenses"]
    end)
    -- RPC.register("varial-mdt:missingCitizen", function(cid, time)
    --     local result = SQL('SELECT * FROM characters WHERE id = @id', {['@id'] = cid})
    ...
    --     end
    -- end)
    -- RPC.register("varial-mdt:getAllMissing", function()
    --     local Tables = {}
    ...
    --     return Tables
    -- end)
    -- RPC.register("varial-mdt:deleteMissing", function(id, time)
    --     local src = source
    ...
    -- end)
    -- RPC.register("varial-mdt:saveMissingInfo", function(id, imageurl, notes)
    ...
    --     })
    -- end)
    -- RPC.register("varial-mdt:searchMissing", function(name)
    --     local ReturnData = {}
    ...
    --     return ReturnData
    -- end)
    -- RPC.register("varial-mdt:getMissingData", function(id)
    ...
    --     end
    -- end)
    RPC.register("varial-mdt:searchBolos", function(searchVal)
        local Matches = {}
        local BolosData = SQL("SELECT * FROM mdt_bolos", {})
        for index = 1, #BolosData, 1 do
    ...
    RPC.register("varial-mdt:getAllBolos", function()
        local Tables = {}
        local BolosData = SQL("SELECT * FROM mdt_bolos", {})
        for index = 1, #BolosData, 1 do
    ...
    RPC.register("varial-mdt:getBoloData", function(id)
        local result = SQL('SELECT * FROM mdt_bolos WHERE id = @id', {['@id'] = id})
        return result[1]
    end)
    RPC.register("varial-mdt:newBolo", function(existing, id, title, plate, owner, individual, detail, tags, gallery, officers, time)
        local src = source
        local Player = exports["varial-base"]:getModule("Player"):GetUser(src)
    ...
    RPC.register("varial-mdt:deleteBolo", function(id, time)
        local src = source
    ...
    RPC.register("varial-mdt:deleteReport", function(id, time)
        local src = source
    ...
    RPC.register("varial-mdt:deleteIncident", function(id, time)
        local src = source
    ...
    RPC.register("varial-mdt:deleteICU", function(id)
        SQL("DELETE FROM mdt_bolos WHERE id = @id", {['@id'] = id})
    end)
    RPC.register("varial-mdt:getAllReports", function()
        local Tables = {}
    ...
    RPC.register("varial-mdt:getReportData", function(id)
        local result = SQL('SELECT * FROM mdt_report WHERE id = @id', {['@id'] = id})
        return result[1]
    end)
    RPC.register("varial-mdt:searchReports", function(name)
        local Matches = {}
    ...
    RPC.register("varial-mdt:newReport", function(existing, id, title, reporttype, detail, tags, gallery, officers, civilians, time)
        local src = source
        if not existing then
    ...
    RPC.register("varial-mdt:searchVehicles", function(name)
        local ReturnData = {}
    ...
    RPC.register("varial-mdt:searchWeapon", function(name)
        local ReturnData = {}
    ...
    RPC.register("varial-mdt:getWeaponData", function(serialnumber)
        local result = SQL('SELECT * FROM weapons_serials WHERE serial = @serial', {['@serial'] = serialnumber})
        if result[1] then
            return result[1]
        end
    end)
    RPC.register("varial-mdt:saveWeaponInfo", function(serialnumber, imageurl, brand, type, notes)
    ...
    RPC.register("varial-mdt:getVehicleData", function(plate)
    ...
    RPC.register("varial-mdt:saveVehicleInfo", function(plate, imageurl, notes)
        local result = SQL('SELECT * FROM mdt_vehicleinfo WHERE plate = @plate', {['@plate'] = plate})
        if result[1] then
    ...
    RPC.register("varial-mdt:knownInformation", function(type, status, plate)
        local result = SQL('SELECT * FROM mdt_vehicleinfo WHERE plate = @plate', {['@plate'] = plate})
        if result[1] then
            if type == "code5" then
    ...
    RPC.register("varial-mdt:getAllLogs", function()
        local MDTLogs = {}
        return MDTLogs
    end)
    RPC.register("varial-mdt:toggleDuty", function(cid, status)
        local src = source
        ChangePlayerDuty(src, status)
    end)
    RegisterServerEvent("varial-mdt:setCallsign", function(cid, callsign)
    ...
    RPC.register("varial-mdt:callDetach", function(callid)
        local src = source
        local NewTable = {}
    ...
    RPC.register("varial-mdt:removeCall", function(callid)
        MDTDispatchData[callid] = nil
        return source, callid
    end)
    RPC.register("varial-mdt:callAttach", function(callid)
        local src = source
    ...
    RPC.register("varial-mdt:attachedUnits", function(callid)
        local ReturnData = {}
        for index, data in pairs(AttachedUnits) do
            if data.CallId == callid then
                local Player = exports["varial-base"]:getModule("Player"):GetUser(data.Source)
                local char = Player:getCurrentCharacter()
                table.insert(ReturnData, {
                    cid = char.id,
                    job = Player.job,
    ...
    RPC.register("varial-mdt:callDispatchDetach", function(callid, cid)
    ...
    RPC.register("varial-mdt:setDispatchWaypoint", function(callid)
        return MDTDispatchData[callid]
    end)
    RPC.register("varial-mdt:callDragAttach", function(callid, cid)
        local Player = GetUserByCid(tostring(cid))
        for _, data in pairs(AttachedUnits) do
            if data.CallId == callid then
     ...
     RPC.register("varial-mdt:sendMessage", function(message, time)
         local src = source
         local Player = exports["varial-base"]:getModule("Player"):GetUser(src)
         local char = Player:getCurrentCharacter()
         local ReturnData = {}
     ...
     RPC.register("varial-mdt:refreshDispatchMsgs", function()
         return GetAllMessages()
     end)
    RPC.register("varial-mdt:setWaypoint:unit", function(cid)
        local Player = GetUserByCid(cid)
        local PlayerCoords = GetEntityCoords(GetPlayerPed(Player.source))
        return PlayerCoords
    end)
    RPC.register("varial-mdt:isLimited", function()
        local Player = exports["varial-base"]:getModule("Player"):GetUser(source)
