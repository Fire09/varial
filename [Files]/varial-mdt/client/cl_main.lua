...
RegisterNetEvent('varial-mdt:RequestOpen')
AddEventHandler('varial-mdt:RequestOpen', function()
	TriggerServerEvent('varial-mdt:OpenMDT')
end)
...
LoadEvanVersion = function()
     ...
     OpenPublicRecords = function()
         ...
         TriggerEvent('varial-mdt:animation')
         local PlayerInfo = RPC.execute("varial-mdt:getInfo")
    ...
    function EnableGUI(enable)
        if enable then 
            TriggerServerEvent('varial-mdt:opendashboard')
            SendNUIMessage({type = "bulletin", data = RPC.execute("varial-mdt:dashboardbulletin")})
            SendNUIMessage({type = "dispatchmessages"})
            SendNUIMessage({type = "warrants", data = RPC.execute("varial-mdt:getWarrants")})
        end
        SetNuiFocus(enable, enable)
        SendNUIMessage({type = "show", enable = enable, job = exports['isPed']:isPed("myjob")})
        isOpen = enable
        TriggerEvent('varial-mdt:animation')
    end
    ...
    function RefreshGUI()
        ...
        SendNUIMessage({type = "show", enable = false, job = exports['isPed']:isPed("myjob")})
        isOpen = false
    end
     RegisterNetEvent('varial-mdt:getActiveUnits')
     AddEventHandler('varial-mdt:getActiveUnits', function(lspd, bcso, sasp, ems)
         SendNUIMessage({ type = "getActiveUnits", lspd = lspd, bcso = bcso, sasp = sasp, ems = ems })
     end)
    AddEventHandler('varial-mdt:animation', function()
        if not isOpen then 
            return 
        end
    ...
    RegisterNUICallback("deleteBulletin", function(data, cb)
        local ignoreId, sentData, job = RPC.execute("varial-mdt:deleteBulletin", data.id)
        if ignoreId == GetPlayerServerId(PlayerId()) then 
            return 
        end
        if job == "police" or job == "state" or job == "sheriff" and exports['isPed']:isPed("myjob") == "police" or exports['isPed']:isPed("myjob") == "state" or exports['isPed']:isPed("myjob") == "sheriff" then
            SendNUIMessage({type = "deleteBulletin", data = sentData})
        elseif job == exports['isPed']:isPed("myjob") then
            SendNUIMessage({type = "deleteBulletin", data = sentData})
        end
    end)
    RegisterNUICallback("updateLicence", function(data, cb)
        ...
        local cid = data.cid
        RPC.execute("varial-mdt:updateLicence", type, status, cid)
    end)
    RegisterNUICallback("newBulletin", function(data, cb)
        local ignoreId, sentData, job = RPC.execute("varial-mdt:newBulletin", data.title, data.info, data.time)
        if ignoreId == GetPlayerServerId(PlayerId()) then 
            return 
        end
        if job == "police" or job == "state" or job == "sheriff" and exports['isPed']:isPed("myjob") == "police" or exports['isPed']:isPed("myjob") == "state" or exports['isPed']:isPed("myjob") == "sheriff" then
    ...
    RegisterNetEvent('varial-mdt:open')
    AddEventHandler('varial-mdt:open', function(job, lastname, firstname)
        open = true
        EnableGUI(open)
        local x, y, z = table.unpack(GetEntityCoords(PlayerPedId()))
        local currentStreetHash, intersectStreetHash = GetStreetNameAtCoord(x, y, z)
        ...
        if job == 'police' then 
            information = "Welcome, Officer " ..lastname
        elseif job == 'state' then 
            information = "Welcome, Trooper " ..lastname
        elseif job == 'sheriff' then 
            information = "Welcome, Deputy " ..lastname
        elseif job == 'ems' then
            information = "Welcome, Medic " ..lastname
        end

        SendNUIMessage({type = "data", name = information, location = playerStreetsLocation, fullname = firstname..' '..lastname, playerjob = job})
    end)
    ...
    RegisterNUICallback("searchProfiles", function(data, cb)
        SendNUIMessage({type = "profiles", data = RPC.execute("varial-mdt:searchProfile", data.name), isLimited = RPC.execute("varial-mdt:isLimited")})
    end)
    RegisterNUICallback("saveProfile", function(data, cb)
        RPC.execute("varial-mdt:saveProfile", data.pfp, data.description, data.id)
    end)
    RegisterNUICallback("newWeapon", function(data, cb)
        RPC.execute("varial-mdt:addWeapon", data.id, data.serialnumber)
    end)
    RegisterNUICallback("getProfileData", function(data, cb)
        local isLimited = RPC.execute("varial-mdt:isLimited")
        local ProfileData = RPC.execute("varial-mdt:getProfileData", data.id)
        if not isLimited then
            ...
            for i = 1, #vehicles do
                ...
                ProfileData['vehicles'][i]['model'] = string.upper(ProfileData['vehicles'][i]['model'])
            end
        end
    ...
    RegisterNUICallback("newTag", function(data, cb)
        if data.id ~= "" and data.tag ~= "" then
            RPC.execute("varial-mdt:newTag", data.id, data.tag)
        end
    end)
    RegisterNUICallback("removeProfileTag", function(data, cb)
        RPC.execute("varial-mdt:removeProfileTag", data.cid, data.text)
    end)
    RegisterNUICallback("addGalleryImg", function(data, cb)
        RPC.execute("varial-mdt:addGalleryImg", data.cid, data.URL)
    end)
    RegisterNUICallback("removeGalleryImg", function(data, cb)
        RPC.execute("varial-mdt:removeGalleryImg", data.cid, data.URL)
    end)
    RegisterNUICallback("searchIncidents", function(data, cb)
        SendNUIMessage({type = "incidents", data = RPC.execute("varial-mdt:searchIncidents", data.incident)})
    end)
    RegisterNUICallback("getIncidentData", function(data, cb)
        local sentData, sentConvictions = RPC.execute("varial-mdt:getIncidentData", data.id)
    ...
    RegisterNUICallback("incidentSearchPerson", function(data, cb)
        SendNUIMessage({type = "incidentSearchPerson", data = RPC.execute("varial-mdt:incidentSearchPerson", data.name)})
    end)
    RegisterNUICallback("searchBolos", function(data, cb)
        SendNUIMessage({type = "bolos", data = RPC.execute("varial-mdt:searchBolos", data.searchVal)})
    end)
    RegisterNUICallback("getAllBolos", function(data, cb)
        SendNUIMessage({type = "bolos", data = RPC.execute("varial-mdt:getAllBolos")})
    end)
    RegisterNUICallback("getAllIncidents", function(data, cb)
        SendNUIMessage({type = "incidents", data = RPC.execute("varial-mdt:getAllIncidents")})
    end)
    RegisterNUICallback("getBoloData", function(data, cb)
        SendNUIMessage({type = "boloData", data = RPC.execute("varial-mdt:getBoloData", data.id)})
    end)
    RegisterNUICallback("jailPlayer", function(data, cb)
        RPC.execute("varial-mdt:JailPlayer", data.cid, data.time)
    end)
    RegisterNUICallback("newBolo", function(data, cb)
        local sentData = RPC.execute("varial-mdt:newBolo", data.existing, data.id, data.title, data.plate, data.owner, data.individual, data.detail, data.tags, data.gallery, data.officers, data.time)
        SendNUIMessage({type = "boloComplete", data = sentData})
    end)
    RegisterNUICallback("deleteBolo", function(data, cb)
        RPC.execute("varial-mdt:deleteBolo", data.id)
    end)
    RegisterNUICallback("deleteIncident", function(data, cb)
        RPC.execute("varial-mdt:deleteIncident", data.id, data.time)
    end)
    RegisterNUICallback("deleteReport", function(data, cb)
        RPC.execute("varial-mdt:deleteReport", data.id, data.time)
    end)
    RegisterNUICallback("deleteICU", function(data, cb)
        RPC.execute("varial-mdt:deleteICU", data.id)
    end)
    RegisterNUICallback("getAllReports", function(data, cb)
        local asdsadassa = RPC.execute("varial-mdt:getAllReports")
        print(json.encode(asdsadassa))
        SendNUIMessage({type = "reports", data = RPC.execute("varial-mdt:getAllReports")})
    end)
    RegisterNUICallback("getReportData", function(data, cb)
        SendNUIMessage({type = "reportData", data = RPC.execute("varial-mdt:getReportData", data.id)})
    end)
    RegisterNUICallback("searchReports", function(data, cb)
        SendNUIMessage({type = "reports", data = RPC.execute("varial-mdt:searchReports", data.name)})
    end)
    RegisterNUICallback("newReport", function(data, cb)
        local sentData = RPC.execute("varial-mdt:newReport", data.existing, data.id, data.title, data.type, data.detail, data.tags, data.gallery, data.officers, data.civilians, data.time)
        SendNUIMessage({type = "reportComplete", data = sentData})
    end)
    RegisterNUICallback("searchVehicles", function(data, cb)
        local sentData = RPC.execute("varial-mdt:searchVehicles", data.name)
        for i = 1, #sentData do
            sentData[i]['plate'] = string.upper(sentData[i]['plate'])
            sentData[i]['model'] = sentData[i]['model']
        end
        SendNUIMessage({type = "searchedVehicles", data = sentData})
    end)
    RegisterNUICallback("searchWeapon", function(data, cb)
        local sentData = RPC.execute("varial-mdt:searchWeapon", data.name)
        SendNUIMessage({type = "searchedWeapons", data = sentData})
    end)
    RegisterNUICallback("getWeaponData", function(data, cb)
        local sentData = RPC.execute("varial-mdt:getWeaponData", data.serialnumber)
        SendNUIMessage({type = "getWeaponData", data = sentData})
    end)
    RegisterNUICallback("getVehicleData", function(data, cb)
        local sentData = RPC.execute("varial-mdt:getVehicleData", data.plate)
        if sentData then
    ...
    RegisterNUICallback("saveVehicleInfo", function(data, cb)
        RPC.execute("varial-mdt:saveVehicleInfo", data.plate, data.imageurl, data.notes)
    end)
    RegisterNUICallback("saveWeaponInfo", function(data, cb)
        RPC.execute("varial-mdt:saveWeaponInfo", data.serialnumber, data.imageurl, data.brand, data.type, data.notes)
    end)
    RegisterNUICallback("knownInformation", function(data, cb)
        RPC.execute("varial-mdt:knownInformation", data.type, data.status, data.plate)
    end)
    RegisterNUICallback("getAllLogs", function(data, cb)
        SendNUIMessage({type = "getAllLogs", data = RPC.execute("varial-mdt:getAllLogs")})
    end)
    RegisterNUICallback("getPenalCode", function(data, cb)
        local titles, penalcode = RPC.execute("varial-mdt:getPenalCode")
        SendNUIMessage({type = "getPenalCode", titles = titles, penalcode = penalcode})
    end)
    -- RegisterNUICallback("missingCitizen", function(data, cb)
    --     RPC.execute("varial-mdt:missingCitizen", data.cid, data.bool)
    -- end)
    RegisterNUICallback("getAllMissing", function(data, cb)
        SendNUIMessage({type = "missing"})
    end)
    -- RegisterNUICallback("deleteMissing", function(data, cb)
    --     RPC.execute("varial-mdt:deleteMissing", data.id, data.time)
    -- end)
    -- RegisterNUICallback("searchMissing", function(data, cb)
    --     local sentData = RPC.execute("varial-mdt:searchMissing", data.name)
    --     SendNUIMessage({type = "searchedMissing", data = sentData})
    -- end)
    -- RegisterNUICallback("getMissingData", function(data, cb)
    --     local sentData = RPC.execute("varial-mdt:getMissingData", data.id)
    --     SendNUIMessage({type = "getMissingData", data = sentData})
    -- end)
    -- RegisterNUICallback("saveMissingInfo", function(data, cb)
    --     RPC.execute("varial-mdt:saveMissingInfo", data.id, data.imageurl, data.notes)
    -- end)
    RegisterNUICallback("toggleDuty", function(data, cb)
        RPC.execute("varial-mdt:toggleDuty", data.cid, data.status)
    end)
    RegisterNUICallback("setCallsign", function(data, cb)
        TriggerServerEvent('varial-mdt:setCallsign', data.cid, data.newcallsign)
    end)
    RegisterNUICallback("saveIncident", function(data, cb)
        RPC.execute("varial-mdt:saveIncident", data.ID, data.title, data.information, data.tags, data.officers, data.civilians, data.evidence, data.associated, data.time)
    end)
    RegisterNUICallback("removeIncidentCriminal", function(data, cb)
        RPC.execute("varial-mdt:removeIncidentCriminal", data.cid, data.incidentId)
    end)
    RegisterNUICallback("setWaypoint", function(data, cb)
        local callInformation = RPC.execute("varial-mdt:setWaypoint", data.callid)
        print(callInformation)
        SetNewWaypoint(callInformation['origin']['x'], callInformation['origin']['y'])
    end)
    RegisterNUICallback("callDetach", function(data, cb)
        local callid, sentData = RPC.execute("varial-mdt:callDetach", data.callid)
        local job = exports['isPed']:isPed("myjob")
        if job == "police" or job == "state" or job == "sheriff" or job == 'ems' then 
    ...
    RegisterNUICallback("removeCall", function(data, cb)
        local src, callid = RPC.execute("varial-mdt:removeCall", data.callid)
    ...
    RegisterNUICallback("callAttach", function(data, cb)
        local callid, sentData = RPC.execute("varial-mdt:callAttach", data.callid)
        local job = exports['isPed']:isPed("myjob")
        if job == "police" or job == "state" or job == "sheriff" or job == 'ems' then
    ...
    RegisterNUICallback("setWaypointU", function(data, cb)
     ...
     RegisterNUICallback("refreshDispatchMsgs", function(data, cb)
         SendNUIMessage({type = "dispatchmessages", data = RPC.execute("varial-mdt:refreshDispatchMsgs")})
     end)
 end
