...
local pillWorker = { ['x'] = 68.896697998047, ['y'] = -1569.8204345703, ['z'] = 28.597768783569,['h'] = 90.23208618164, ['info'] = 'boop bap' }
...
local carspawns = {
	[1] =  { ['x'] = 63.9225, ['y'] = 16.2337, ['z'] = 69.1292,['h'] = 339.0545, ['info'] = ' car 8' },
	[2] =  { ['x'] = 60.6167, ['y'] = 16.8838, ['z'] = 69.1564,['h'] = 339.0545, ['info'] = ' car 1' },
	[3] =  { ['x'] = 57.4580, ['y'] = 18.1933, ['z'] = 69.1564,['h'] = 339.0545, ['info'] = ' car 2' },
}
	...
	while tasking do
		...
		if dstcheck < 2.0 and pedCreated then
			...
			if IsControlJustReleased(0,38) then
				...
				DoDropOff()
				tasking = false
				TriggerEvent('varial-dispatch:drugdealreport')
			end
		end
	end
...
Citizen.CreateThread(function()
	exports["varial-polyzone"]:AddBoxZone("oxy_run_start", vector3(114.38, -4.85, 67.81), 1, 1, {
		name="oxy_run_start",
		heading=22,
		--debugPoly=true,
		minZ=65.81,
...
AddEventHandler('varial-polyzone:enter', function(name)
    if name == "oxy_run_start" then
        ...
        OxyLocation()
		if not OxyRun then
			exports["varial-interaction"]:showInteraction("[E] Start your run! ($750)")
		else
...
-- 					print("oxy run started")          
-- 			end
-- 		end
-- 	end)
-- end

function OxyLocation()
	Citizen.CreateThread(function()
        while OxySpot do
            Citizen.Wait(5)
			if IsControlJustReleased(0, 38) then
		...
		elseif OxyRun then
			if (not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 100.0) and vehspawn then
				...
				if tasking then
				    ...
				    salecount = salecount + 1
				    if salecount == 10 then
						TriggerEvent("DoLongHudText", "Finished Run.")
				    	OxyRun = false
				    end
				end
			end
...
function DoDropOff(requestMoney)
	local bandprice = math.random(150, 200)
	local rollcashprice = math.random(30, 50)
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		if OxyRun then
			cashPayment = math.random(100,150)
			if exports["varial-inventory"]:hasEnoughOfItem("rollcash",1,false) then     
				TriggerEvent("inventory:removeItem","rollcash", 1)   
				cashPayment = cashPayment + rollcashprice             
				TriggerEvent("DoLongHudText","Thanks for the extra sauce!")
			elseif exports["varial-inventory"]:hasEnoughOfItem("band",10,false) then     
	...
	if success then
		...
		if math.random(15) == 6 then
			TriggerEvent("player:receiveItem", "safecrackingkit", 1)
		end

		TriggerServerEvent('mission:finished', cashPaymentFinal)
...
--         Citizen.Wait(100)
--     end
-- 	drugStorePed = CreatePed(pedType, hashKey, pillWorker["x"],pillWorker["y"],pillWorker["z"], 90.0, 1, 1)
-- 	SetEntityHeading(drugStorePed, 150)
