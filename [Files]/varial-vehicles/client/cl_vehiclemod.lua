...
function preventVehicleExit()
    Citizen.CreateThread(function()
        ...
        disableControl = true
        disableTurning()
        local finished = exports["erp-taskbar"]:taskBar(options[math.random(1,3)],"Taking off Harness",true)
...
function toggleHarness()
    ...
    local finished = 0
    if harness then
        finished = exports["erp-taskbar"]:taskBar(5000,"Taking off Harness",true)
    else
        finished = exports["erp-taskbar"]:taskBar(5000,"Putting on Harness",true)
    end
    if (finished == 100) then
...
AddEventHandler('fakeplate:accepted', function(newplate, oldplate)
    local enough = exports["varial-inventory"]:hasEnoughOfItem("fakeplate",1)
    if not LicencePlate.Index and not LicencePlate.Number then
        ...
        if Distance < 3.5 and not IsPedInAnyVehicle(PlayerPed, false) then
            ...
            local finished = exports["varial-taskbar"]:taskBar(15000,"Changing plates", true)
            if finished == 100 then
...
AddEventHandler('fakeplate:off', function()
		...
		if (Vehicle == LastVehicle) then
            ...
            TaskPlayAnim(PlayerPed,"amb@medic@standing@kneel@base","base", 8.0, -8, -1, 6, 0, 0, 0, 0)
            FreezeEntityPosition(PlayerPed, true)
            local finished = exports["varial-taskbar"]:taskBar(15000,"Changing back to orginal plates", true)
            FreezeEntityPosition(PlayerPed, false)
			-- Wait 6 seconds
			Citizen.Wait(6000)
            -- POG SHIT WORKS NOW 
