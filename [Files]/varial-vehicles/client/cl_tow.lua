...
AddEventHandler('pv:tow', function()
	...
	if isVehicleTow then
		...
		if not currentlyTowedVehicle then
		    ...
		    local finished = exports["varial-taskbar"]:taskBar(15000,"Hooking up vehicle.")
        	if finished == 100 and aDist < 5.0 then
			
				if targetVehicle ~= 0 then
					TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck2', 0.5)
					local part2 = exports["varial-taskbar"]:taskBar(7000,"Towing Vehicle")
		    ...
		    TriggerEvent("animation:tow")
		    TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck', 0.5)
		    local finished = exports["varial-taskbar"]:taskBar(7000,"Untowing Vehicle")
        	if finished == 100 and aDist < 2.0 then

        		TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck2', 0.5)
        		local part2 = exports["varial-taskbar"]:taskBar(7000,"Unhooking Vehicle")
...
RegisterNetEvent("varial-vehicles:repo:success")
AddEventHandler("varial-vehicles:repo:success", function()
...
RegisterNetEvent("varial-vehicles:repo:success2")
AddEventHandler("varial-vehicles:repo:success2", function()
	local veh = GetVehiclePedIsIn(PlayerPedId())
	if veh ~= 0 then
		DeleteVehicle(veh)
		DeleteEntity(veh)
...
AddEventHandler("search:list:repo", function()
	...
	if job == 'pdm' or job == 'towunion' or job == 'sanders' then
        ...
        if valid then
            TriggerServerEvent("varial-vehicles:checkrepo", valid[1].input)
        end
	else
...
AddEventHandler("search:list:showJudgeRepo", function()
	...
	if job == 'judge' then
        ...
        if valid then
            TriggerServerEvent("varial-vehicles:judgerepo", valid[1].input)
        end
	else
		TriggerEvent('DoLongHudText', 'You cannot do this!', 2)
	end
...
RegisterNetEvent("varial-vehicles:towBill")
AddEventHandler("varial-vehicles:towBill", function(amount)
