...
RegisterServerEvent("varial-vehicles:checkrepo")
AddEventHandler("varial-vehicles:checkrepo", function(plate)
    local src = source
...
-- RegisterServerEvent("varial-vehicles:repo")
-- AddEventHandler("varial-vehicles:repo", function(plate)
--     local src = source
-- 	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
--                         ['@repoed'] = "1"
--                     })
--                     TriggerClientEvent("varial-vehicles:repo:success", src)
...
-- RegisterServerEvent("varial-vehicles:judgerepo")
-- AddEventHandler("varial-vehicles:judgerepo", function(plate)
--     local src = source
-- 	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
-- RegisterServerEvent("varial-vehicles:judgerepodelete")
-- AddEventHandler("varial-vehicles:judgerepodelete", function(plate)
--     local src = source
-- 	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
--     exports.oxmysql:execute("SELECT `license_plate`, `repoed` FROM `characters_cars` WHERE license_plate = ?", {char.id}, function(data)
...
-- RegisterServerEvent("varial-vehicles:repo2")
-- AddEventHandler("varial-vehicles:repo2", function(plate)
--     local src = source
-- 	local user = exports["varial-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
--                     ['@vehicle_state'] = "In"
--                 })
--                 TriggerClientEvent("varial-vehicles:repo:success2", src)
--             else
--                 TriggerClientEvent("varial-vehicles:repo:success2", src)
--             end
--         end
...
-- RegisterServerEvent("varial-vehicles:release:vehicle")
-- AddEventHandler("varial-vehicles:release:vehicle", function(ServerID, plate)
...
--         ['repoed'] = "0"
--     })
--     exports['varial-exports']:removeCash(SrcID, 500)
...
RegisterCommand('towbill', function(source, args)
	...
	exports.oxmysql:execute('SELECT * FROM jobs_whitelist WHERE cid = ?', {character2.id}, function(result)
		if result[1].job == 'towunion' then
			...
			TriggerClientEvent("varial-vehicles:towBill", src, tonumber(args[2]))
		else
		end
	end)
end)