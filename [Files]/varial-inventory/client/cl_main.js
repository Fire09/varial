...
RegisterNetEvent('varial-base:playerSpawned');
on('varial-base:playerSpawned', async (broughtData) => {
...
//  $.post("http://varial-inventory/SlotJustUsed", JSON.stringify({target: targetSlot, origin: originSlot, itemid: itemidsent }));
let recentused = [];
...
on('varial-items:SetAmmo', (sentammoTable) => {
    if (sentammoTable) {
...
async function CloseGui(pIsItemUsed = false) {
    ...
    openedInv = false;
    recentused = [];
    emit('varial-inventory:shitlord-check')
}
...
function SetCustomNuiFocus(hasKeyboard, hasMouse) {
    ...
    //   if (HasNuiFocus === true) {
    //   	emit("varial-voice:focus:set", HasNuiFocus, hasKeyboard, hasMouse);
    //   } else {
    // 	  setTimeout(() => {if (HasNuiFocus !== true) emit("varial-voice:focus:set", false, false, false);}, 1000)
    //   }
}
