...
$(document).ready(function() {
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://varial-inventory/UpdateSettings',
            JSON.stringify({
    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.response == 'openGui') {
            ...
            if (itemList[item.id]) {
                $.post(
                    'https://varial-inventory/GiveItem',
                    JSON.stringify([
                        item.id,
                        item.amount,
...
function invStack(
    ...
    $.post('https://varial-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://varial-inventory/move', JSON.stringify(arr));
}
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory, itemid1, metainformation1, itemid2, metainformation2) {
    ...
    $.post('https://varial-inventory/swap', JSON.stringify(arr));
}

function removeCraftItems(itemid, moveAmount, CraftArrayID) {
    let arr = itemList[itemid].craft[CraftArrayID];
    let amount = moveAmount;
    $.post('https://varial-inventory/removeCraftItems', JSON.stringify([arr, amount]));
}
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://varial-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            // $.post('https://varial-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function DropItem(slot, amountDropped) {
    ...
    // $.post('https://varial-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
    // $.post('https://varial-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://varial-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://varial-inventory/SlotJustUsed',
        JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://varial-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function InsertItem(targetSlot, originSlot, inv1, inv2, itemid1, iteminfo1, itemid2, iteminfo2) {
    ...
    $.post(
        'https://varial-inventory/insert-item',
        JSON.stringify({
            originInventory,
            targetInventory,
            originSlot,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://varial-inventory/ServerCloseInventory',
        JSON.stringify({
...
function CheckCraftFail(itemid, moveAmount) {
    ...
    for (let ArrayX = 0; ArrayX < requirementsArray.length; ArrayX++) {
        ...
        if (!itemcheck && !weightcheck) {
            ...
            $.post('https://varial-inventory/craftProgression', JSON.stringify({
...
function useitem() {
    ...
    if (currentInventory == 1) {
        ...
        $.post('https://varial-inventory/invuse', JSON.stringify(arr));
