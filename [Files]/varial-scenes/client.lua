...
RegisterNetEvent('varial-scenes:send', function(sent)
    scenes = sent
end)
...
RegisterCommand('-scenecreate', function()
    ...
    local scene = exports["varial-applications"]:KeyboardInput({
        header = "Add Scene",
        rows = {
            {
                id = 0,
    ...
    TriggerServerEvent('varial-scenes:add', coords, message, color, distance)
end)
...
RegisterCommand('+scenedelete', function()
    ...
    if scene ~= nil then
        TriggerServerEvent('varial-scenes:delete', scene)
    end
end)
...
TriggerServerEvent('varial-scenes:fetch')
