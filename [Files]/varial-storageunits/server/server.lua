RPC.register("varial-storageunits:get", function()
    return storage_units
end)


RegisterServerEvent("varial-storageunits:purchase", function(args)
    local pSrc = source
    ...
    if user:getBalance() >= purchase_price then
        ...
        exports.oxmysql:execute("INSERT INTO storage_units (id, data) VALUES (@id, @data)", {
            ['@id'] = args.storage_unit_id,
            ['@data'] = json.encode(table)
        })

        TriggerClientEvent("DoLongHudText", pSrc, 'Congratulations, you now own storage unit ' ..args.storage_unit_id)
    else
        TriggerClientEvent("DoLongHudText", pSrc, 'Purchase failed, you dont have enough money for this.', 2)
    end
end)

RPC.register("varial-storageunits:occupied", function(storage_unit_id)
    local pSrc = source
    local user = exports["varial-base"]:getModule("Player"):GetUser(pSrc)
    local char_id = user:getCurrentCharacter().id
    exports.oxmysql:execute("SELECT * FROM storage_units WHERE id = @id", {['id'] = storage_unit_id}, function(result)
        if result[1] ~= nil then
            ...
            local menu_data = {
                ...
                },
                {
                    ...
                    params = {
                        event = "varial-storageunits:attempt:inventory",
                        args = {
                            id = storage_unit_id,
                        }
                    }
            ...
            
                {
                    ...
                    params = {
                        event = "varial-storageunits:attempt:access",
                        args = {
                            id = storage_unit_id,
            ...
            if tonumber(storage_owner) == char_id then
                TriggerClientEvent('varial-context:sendMenu', pSrc, menu_data)  
            else
                for i=1, #keys do
            ...
            local menu_data = {
                ...
                },
                {
                    ...
                    params = {
                        event = "varial-storageunits:attempt:purchase",
                        args = {
                            id = storage_unit_id,
                            price = 70000
                        }
...
RPC.register("varial-storageunits:give:access", function(storage_unit_id, given_cid)
    local pSrc = source
    exports.oxmysql:execute("SELECT `given_access` FROM storage_units WHERE id = @id", {['id'] = storage_unit_id}, function(result)
        ...
        if update_keys ~= "{}" then
            ...
            exports.oxmysql:execute("UPDATE storage_units SET `given_access` = @given_access WHERE `id` = @id", {
...
RPC.register("varial-storageunits:remove:access", function(storage_unit_id, given_cid, given_name)
    local pSrc = source
...
RegisterServerEvent("varial-storageunit:view", function(storage_unit_id)
    local pSrc = source
    local user = exports["varial-base"]:getModule("Player"):GetUser(pSrc)
    exports.oxmysql:execute("SELECT `given_access` FROM storage_units WHERE id = @id", {['id'] = storage_unit_id}, function(result)
        if result[1].given_access ~= nil then
            ...
            for i=1, #current_keys do  
                exports.oxmysql:execute("SELECT `first_name`, `last_name`, `id` FROM characters WHERE id = @id", {['id'] = current_keys[i]}, function(result)
                    if result[1] ~= nil then
                        for i=1, #result do
                            TriggerClientEvent('varial-context:sendMenu', pSrc, {
                                {
                                    ...
                                    txt = "CID: " ..result[i].id,
                                    params = {
                                        event = "varial-storageunit:attempt:remove",
                                        args = {
                                            id = storage_unit_id,
...
function get_storage_owner(pSrc, storage_unit_id)
    local user = exports["varial-base"]:getModule("Player"):GetUser(pSrc)
    local char_id = user:getCurrentCharacter().id
    exports.oxmysql:execute("SELECT `data` FROM storage_units WHERE id = @id", {['id'] = storage_unit_id}, function(result)
        if result[1] ~= nil then
            local storage_data = json.decode(result[1].data)
...
RPC.register("varial-storageunits:attenpt:transfer", function(storage_unit_id, transfer_cid)
    local pSrc = source
    local user = exports["varial-base"]:getModule("Player"):GetUser(pSrc)
    local char_id = user:getCurrentCharacter().id
    ...
    exports.oxmysql:execute("SELECT `data` FROM storage_units WHERE id = @id", {['id'] = storage_unit_id}, function(result)
        if result[1] ~= nil then
            ...
            exports.oxmysql:execute("UPDATE storage_units SET `data` = @data WHERE `id` = @id", {
