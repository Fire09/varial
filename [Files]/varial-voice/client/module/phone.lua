	...
	Citizen.CreateThread(function()
		...
		while callChannel ~= 0 do
			...
			if NetworkIsPlayerTalking(PlayerId()) and not changed then
				...
				TriggerServerEvent('varial-voice:setTalkingOnCall', true)
			elseif changed and NetworkIsPlayerTalking(PlayerId()) ~= 1 then
				changed = false
				MumbleClearVoiceTargetPlayers(1)
				TriggerServerEvent('varial-voice:setTalkingOnCall', false)
			end
			Wait(0)
		end
	end)
...
RegisterNetEvent('varial-voice:syncCallData', function(callTable, channel)
	callData = callTable
...
RegisterNetEvent('varial-voice:setTalkingOnCall', function(tgt, enabled)
	if tgt ~= playerServerId then
...
RegisterNetEvent('varial-voice:addPlayerToCall', function(plySource)
	callData[plySource] = false
end)

RegisterNetEvent('varial-voice:removePlayerFromCall', function(plySource)
...
function setCallChannel(channel)
	...
	TriggerServerEvent('varial-voice:setPlayerCall', channel)
	callChannel = channel
...
RegisterNetEvent('varial-voice:clSetPlayerCall', function(_callChannel)
	if GetConvarInt('voice_enablePhones', 1) ~= 1 then return end
