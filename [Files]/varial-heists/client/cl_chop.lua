...
Citizen.CreateThread(function()
    exports["varial-polyzone"]:AddBoxZone("chop_zone", vector3(1359.1, -2097.81, 52.0), 15, 15, {
    heading=40,
    --debugPoly=true,
    minZ=51.0,
    maxZ=55.0
...
AddEventHandler("varial-polyzone:enter", function(zone)
    if zone == "chop_zone" and GetVehiclePedIsIn(PlayerPedId(), false) == EvanChopCar then
        ...
        IsInChopZone = true
        TriggerEvent('varial-phone:SendNotify', "Get out of the vehicle and begin to Chop" , 'JobNotify')
        EvanStartChopping()
        exports['varial-interaction']:showInteraction('[E] Start Chopping')
    end
end)

function EvanStartChopping()
	Citizen.CreateThread(function()
...
function ChopVehicleTyre(vehicle, boneID, tyreIndex)
    ...
    local finished = exports["varial-taskbar"]:taskBar(10000,"Slicing Vehicle Tyre")
...
function ChopVehicleDoor(vehicle, boneID, doorIndex)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["varial-taskbar"]:taskBar(14000,"Messing With Door")
...
function ChopVehicleRemains(vehicle, boneID)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["varial-taskbar"]:taskBar(25000,"Seeing Whats Left ..")
    ...
    TriggerEvent('varial-chop:cooldown')
    return success
end

RegisterNetEvent('varial-chop:cooldown')
AddEventHandler('varial-chop:cooldown', function()
    CoolDownActive = true
    Citizen.Wait(900000)
...
function ChopVehiclePart(vehicle)
    ...
    if success then
        TriggerServerEvent("varial-chopshop:vehicle_loot", bone.type)
        TriggerServerEvent('mission:finished', cashPaymentBad)
...
RegisterNetEvent('varial-chopshop:clock-in')
AddEventHandler('varial-chopshop:clock-in', function()
    if CoolDownActive then
        TriggerEvent('DoLongHudText', 'You have a active cooldown!', 2)
    elseif ClockedIntoChop then
        TriggerEvent('DoLongHudText', 'Youre already clocked in', 2)
    else
        ClockedIntoChop = true
        EvanCreateChopCar()
        TriggerEvent('phone:addnotification', 'Illegal Car Company', 'Clocked In, I set your GPS get the car and chop it up!')
    end
end)

RegisterNetEvent('varial-chopshop:clock-out')
AddEventHandler('varial-chopshop:clock-out', function()
    if ClockedIntoChop then
        ClockedIntoChop = false
        TriggerEvent('DoLongHudText', 'Clocked Out !', 2)
        DeleteBlip(blipPickup)
    else
        TriggerEvent('DoLongHudText', 'You was never Clocked In', 2)
    end
end)

-- Ped

function SetChopShopClockInPed()
    modelHash = GetHashKey("a_m_y_smartcaspat_01")
    RequestModel(modelHash)
    while not HasModelLoaded(modelHash) do
        Wait(1)
    end
    created_ped = CreatePed(0, modelHash , 830.30767822266,-2171.7758789062,30.274047851562  -1, true)
    FreezeEntityPosition(created_ped, true)
    SetEntityHeading(created_ped, 272.1259765625)
    SetEntityInvincible(created_ped, true)
    SetBlockingOfNonTemporaryEvents(created_ped, true)
    TaskStartScenarioInPlace(created_ped, "WORLD_HUMAN_SMOKING_POT", 0, true)
end

Citizen.CreateThread(function()
    SetChopShopClockInPed()
end)
...
function EvanCreateChopCar()
	if DoesEntityExist(EvanChopCar) then
	    SetVehicleHasBeenOwnedByPlayer(EvanChopCar,false)
		SetEntityAsNoLongerNeeded(EvanChopCar)
		DeleteEntity(EvanChopCar)
	end

    local EvanChopVehicle = GetHashKey(ChopCars[math.random(#ChopCars)])
    RequestModel(EvanChopVehicle)
    while not HasModelLoaded(EvanChopVehicle) do
        Citizen.Wait(0)
    end
    ...
    SetNewWaypoint(x, y)
    TriggerEvent('varial-phone:SendNotify', "Find the vehicle to chop" , 'JobNotify')
    CreatePickupCarBlip(x, y, z)

    EvanChopCar = CreateVehicle(EvanChopVehicle, x, y, z, h, true, false)

	local pos = GetEntityCoords(EvanChopCar, false)
    Citizen.CreateThread(function()
        while true do
          Citizen.Wait(5)
          if GetVehiclePedIsIn(PlayerPedId(), false) == EvanChopCar then
                SetNewWaypoint(1360.2725830078,-2095.0549316406,51.993408203125)
                TriggerEvent('varial-phone:SendNotify', "Drive to the GPS and begin Chopping" , 'JobNotify')
                DeleteBlip(blipPickup)
            return
          end
        end
