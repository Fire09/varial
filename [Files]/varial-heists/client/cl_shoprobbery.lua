...
AddEventHandler("store:dosafe", function()
	...
	if exports["varial-duty"]:LawAmount() >= 1 then
		...
		TriggerEvent('DoLongHudText', 'There are not enough police to rob this!', 2)
	end
end)
...
AddEventHandler("store:register", function(storeid,regid)
	if exports["varial-duty"]:LawAmount() >= 2 then
		...
		TriggerServerEvent("police:camrobbery",storeid)
		TriggerEvent("client:newStress",true,200)	
		TriggerEvent("safecracking:loop",4,"robbery:register")
		TriggerServerEvent('store:robbery:register2', storeid, regid)
		fuck = storeid
		you = regid
		isLockpicking = true
	else
...
RegisterNetEvent("varial-robberies:HitSafes")
AddEventHandler("varial-robberies:HitSafes", function()
	local storeid = isStoreRobbery()
	if storeid ~= 0 then
...
RegisterNetEvent("varial-robberies:HitRegister")
AddEventHandler("varial-robberies:HitRegister", function()
	...
	if storeid ~= 0 then
		...
		if #(GoodSpot - curCoords) > 1.0 then
			TriggerEvent('DoLongHudText', 'You must be at the front of the register!', 2)
			isLockpicking = false
			return
		end
		if RegisterObject then		
			...
			if objHead - plyHead > 20.0 and objHead - plyHead < 340.0 then
				TriggerEvent('DoLongHudText', 'You must face the register!', 2)
				isLockpicking = false
				return
			end
		...
		TriggerEvent('DoLongHudText', 'You must wait for the other register to open!', 2)
	end
end
end)
...
AddEventHandler("robbery:register", function()
	...
	isLockpicking = false
	isActive = false
	TriggerEvent("DoLongHudText" ,'Please wait 1 minute for it to open do not leave the store !', 1)	
	Citizen.Wait(60000)
	isActive = true
	local safeCoords = safeLocations[storeid]
	...
	TriggerEvent("player:receiveItem","rollcash", math.random(5, 15))
else
...
AddEventHandler("robbery:safe", function()
	...
	TriggerEvent("DoLongHudText" ,'Please wait a few minutes for it to open do not leave the store !', 1)	
	Citizen.Wait(100000)
	local safeCoords = safeLocations[storeid]
	if #(vector3(safeCoords["x"],safeCoords["y"],safeCoords["z"]) - GetEntityCoords(PlayerPedId())) < 14.0 then
	if mathfunc < 25 then
...
camNumber = tonumber(args[1])
	...
	if myJob == "police" or myJob == "state" or myJob == "sheriff" then
		...
		if args[1] == "off" then
			...
			TriggerEvent('DoLongHudText', 'Accesing Camera Number ' .. camNumber, 1)
		end
	else
		TriggerEvent('DoLongHudText', 'Invalid Permissions Attempted to Access Camera ' .. camNumber .. ' | Job Required: Police, Government Alerted', 2)
	end
end)
