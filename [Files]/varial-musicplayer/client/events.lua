RegisterNUICallback("data_status", function(data)
    if soundInfo[data.id] ~= nil then
        if data.type == "finished" then
            ...
            TriggerEvent("varial-musicplayer:songStopPlaying", data.id)
        end
...
RegisterNetEvent("varial-musicplayer:stateSound")
AddEventHandler("varial-musicplayer:stateSound", function(state, data)
