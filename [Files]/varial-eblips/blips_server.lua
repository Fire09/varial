...
RegisterNetEvent('varial-eblips:server:registerSourceName')
AddEventHandler('varial-eblips:server:registerSourceName', function(source, name)
    if not source_cache[source] then source_cache[source] = { } end
    source_cache[source].name = name
end)

RegisterNetEvent('varial-eblips:server:registerPlayerBlipGroup')
AddEventHandler('varial-eblips:server:registerPlayerBlipGroup', function(source, group)
...
RegisterNetEvent('varial-eblips:server:removePlayerBlipGroup')
AddEventHandler('varial-eblips:server:removePlayerBlipGroup', function(source, group)
...
RegisterNetEvent('varial-eblips:server:registerPlayerTempBlipGroup')
AddEventHandler('varial-eblips:server:registerPlayerTempBlipGroup', function(source, group, time)
    TriggerEvent('varial-eblips:server:registerPlayerBlipGroup', source, 'police')
    SetTimeout(time, function()
        TriggerEvent('varial-eblips:server:removePlayerBlipGroup', source, 'police')
    end)
end)
            ...
            Citizen.CreateThread(function()
                for source, is_valid_member in pairs(blip.members) do
                    if is_valid_member then -- more sanity checking
                        TriggerClientEvent('varial-eblips:client:syncMyBlips', source, blips_data)
                        Citizen.Wait(100)
                    end
                end
            end)
