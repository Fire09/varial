...
RegisterNetEvent('varial-banking:UpdatedAccounts')
AddEventHandler('varial-banking:UpdatedAccounts', function(job)
...
function openGui(a)
  ...
  TriggerServerEvent('varial-banking:GetCharacterBanking', cid)
...
RegisterNetEvent('varial-banking:ResetUI')
AddEventHandler('varial-banking:ResetUI', function()
  SetNuiFocus(false, false)
  SendNUIMessage({openBank = false})
  bankOpen = false
  atmOpen = false
    ...
    if IsNearATM() then
      if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
          ...
          if ( atmuse ) then 
              ...
              atmuse = false
              local finished = exports["varial-taskbar"]:taskBar(3000,"Retrieving Card")
              ClearPedTasks(PlayerPedId())
          else
              atmuse = true
        ...
        if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
            ...
            if ( atmuse ) then 
                ...
                atmuse = false
                local finished = exports["varial-taskbar"]:taskBar(1000,"Retrieving Card")
                ClearPedTasks(PlayerPedId())
            else
                atmuse = true
                TaskPlayAnim( player, "mp_common", "givetake1_a", 1.0, 1.0, -1, 49, 0, 0, 0, 0 )
...
RegisterNetEvent("varial-base:addedMoney")
AddEventHandler("varial-base:addedMoney", function(amt, total)
  TriggerEvent("banking:updateCash", total)
  TriggerEvent("banking:addCash", amt)
end)

RegisterNetEvent("varial-base:removedMoney")
AddEventHandler("varial-base:removedMoney", function(amt, total)
  TriggerEvent("banking:updateCash", total)
