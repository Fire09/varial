...
RegisterServerEvent('varial_luckywheel:getLucky')
AddEventHandler('varial_luckywheel:getLucky', function(money)
    local src = source
    ...
    if not isRoll then
        if user ~= nil then
            if user:getCash() >= amount  then
                ...
                -- TriggerEvent('rs_addonaccount:getSharedAccount', 'society_diamondcasino', function(account)
                ...
                SetTimeout(7000, function()
                    ...
                    TriggerClientEvent("varial_luckywheel:rollFinished", -1)
                end)
                TriggerClientEvent("varial_luckywheel:doRoll", -1, _priceIndex)
            else
                TriggerClientEvent("varial_luckywheel:rollFinished", -1)  
...
RegisterServerEvent("attempt:spin", function()
    local pSrc = source
    if CasinoSpin then
        TriggerClientEvent("varial_luckywheel:spinit", pSrc, true)
    else
