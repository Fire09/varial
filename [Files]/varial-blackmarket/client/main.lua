...
RegisterNUICallback("order", function(data)
    ...
    TriggerServerEvent('varial-blackmarket:makeorder', data)
end)
...
RegisterNetEvent("varial-blackmarket:open")
AddEventHandler("varial-blackmarket:open",function()
    if exports['varial-inventory']:hasEnoughOfItem('blackmarket', 1) then
        Animation()
        open(true)
    else
        print('false')
