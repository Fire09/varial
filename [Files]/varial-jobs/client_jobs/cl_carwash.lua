			...
			for i = 1, #vehicleWashStation do
				...
				if #(GetEntityCoords(PlayerPedId()) - vector3(garageCoords2[1], garageCoords2[2], garageCoords2[3])) < 5 then
					if isCleaning == false then
						exports['varial-interaction']:showInteraction("Press [ENTER] to clean your vehicle!")
					else
						exports['varial-interaction']:hideInteraction()
					end
					if(IsControlJustPressed(1, Key)) and isCleaning == false then
						TriggerServerEvent('varial-carwash:checkmoney')
					end
				end
			end
...
RegisterNetEvent('varial-carwash:success')
AddEventHandler('varial-carwash:success', function(price)
	TriggerEvent('DoLongHudText', 'Vehicle Clean $' .. price ..'!')
	FreezeEntityPosition(GetVehiclePedIsUsing(PlayerPedId()), true)
	isCleaning = true
	local finished = exports["varial-taskbar"]:taskBar(10000,"Cleaning Vehicle",false,false,playerVeh)
...
RegisterNetEvent('varial-carwash:notenoughmoney')
AddEventHandler('varial-carwash:notenoughmoney', function(moneyleft)
