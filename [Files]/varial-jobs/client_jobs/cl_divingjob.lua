...
RegisterNetEvent('BeginDivingjob')
AddEventHandler('BeginDivingjob', function()
    local timer = 5000
    local finished = exports["varial-taskbar"]:taskBar(timer,"Checking your cash")
    if finished then
        TriggerServerEvent('varial-scuba:checkAndTakeDepo')
    end
end)


RegisterNetEvent('getDiveingjob')
AddEventHandler('getDiveingjob', function()
  TriggerEvent('varial-jobs:SpawnDivingBoat')
  Wait(2000)
  print('HERE')
  StartDive()
end)
...
function StartDive()
    ...
    if active['active'] == 1 then
        ...
        addBlip(rand)
        if run == 0 then
            TriggerEvent("phone:addnotification", "Diving","Yo, So I've got a location for you. It should be marked on your GPS... I think its a " .. tostring(active['info']))
        elseif run == 1 then
            TriggerEvent("phone:addnotification", "Diving","Yo, So I've got another location for you. It should be marked on your GPS... I think its a " .. tostring(active['info']))
        elseif run == 2 then
            TriggerEvent("phone:addnotification", "Diving","Yo, So I've got another location for you. It should be marked on your GPS... I think its a " .. tostring(active['info']))
        elseif run == 3 then
            TriggerEvent("phone:addnotification", "Diving","Yo, So I've got another location for you. It should be marked on your GPS... I think its a " .. tostring(active['info']))
                ...
                if dis <= 20 and value['active'] == 1 then
                    ...
                    if dis <= 5 then
                        if IsControlJustReleased(0, 38) then
                            ...
                            local waiter = math.random(5000,8500)
                            local finished = exports["varial-taskbar"]:taskBar(waiter,"Searching For Treasure")
                            if finished == 100 then
                            value['active'] = 0
                            scavanged = scavanged + 1
                            FreezeEntityPosition(PlayerPedId(), false)
                            ClearPedTasksImmediately(PlayerPedId())
                            TriggerServerEvent('varial-scuba:findTreasure')
...
function SellItems()
    if exports["varial-inventory"]:hasEnoughOfItem("ironbar",10,false) then 
        TriggerEvent("inventory:removeItem", "ironbar", 10)
        TriggerServerEvent('varial-scuba:paySalvage', 250)
    else
        TriggerEvent('DoLongHudText', 'You dont have enough Material in your pockets to sell!', 2)
    end
end

RegisterNetEvent('varial-jobs:SpawnDivingBoat')
AddEventHandler('varial-jobs:SpawnDivingBoat', function()
	Citizen.Wait(1)
	...
	local spawned_car = CreateVehicle(vehicle,  -1617.0656738281, 5270.9838867188, 3.4332039356232, true, false)
    ...
    -- TriggerEvent('phone:addnotification', 'Diving', "Head to the wreckage site marked on your gps and scalvage whatever you can !")
end)