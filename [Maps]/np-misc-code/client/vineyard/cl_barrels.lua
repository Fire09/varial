  ...
  if p1.action == "add_grapes" then
    ...
    for _, item in pairs(items) do
      ...
      if info.status ~= "dead" then
        ...
        newData.grapes[info.class][info.type] = newData.grapes[info.class][info.type] + item.amount
        TriggerEvent("inventory:removeItemBySlot", "vineyardgrapes", item.amount, item.slot)
        Wait(250)
      end
    end
    RPC.execute("np-vineyard:changeBarrelData", objectId, newData)
