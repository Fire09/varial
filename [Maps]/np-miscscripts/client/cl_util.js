class AnimationTask {
    ...
    abort() {
        if (this.active) {
            exports['varial-taskbar'].taskCancel();
            this.stop();
        }
    }
}
...
function Taskbar(pLength, pName, pRunCheck = false) {
    return new Promise((resolve) => {
        if (pName) {
            exports['varial-taskbar'].taskBar(pLength, pName, pRunCheck, true, null, false, resolve);
        }
        else {
...
function TaskbarMiniGame(pDifficulty, pGap) {
    return new Promise((resolve) => {
        exports['varial-ui'].taskBarSkill(pDifficulty, pGap, resolve);
    });
}